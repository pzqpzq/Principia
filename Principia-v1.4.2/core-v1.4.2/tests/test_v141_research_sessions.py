...
def test_data_discovery_home_is_flat_named_and_reports_empty_state(tmp_path: Path) -> None:
    source = tmp_path / "source"
    source.mkdir()
    app = Principia.open(working_directory=tmp_path / "working")
    try:
        app.repository.register_source(
            "src:materials",
            source,
            "external://materials",
            "Encapsulant cure kinetics",
        )
        home = app.research_sessions.create_data_discovery_home(
            source_ids=["src:materials"],
            title="Discovery · Encapsulant cure kinetics",
            provider_profile_id="siliconflow",
            model="deepseek-ai/DeepSeek-V4-Flash",
        )
        assert home["project_id"] is None
        [listed] = app.research_sessions.sessions()
        assert listed["display_title"] == "Encapsulant curing"
        assert "curing" in listed["summary"].lower()
        assert listed["source_labels"] == ["Encapsulant cure kinetics"]
        assert listed["is_empty"] is True
        assert listed["content_counts"]["graph"] == 0
        # Older projects stored a status placeholder in the descriptive field.
        with app.repository.connect() as conn:
            conn.execute("UPDATE research_sessions SET summary=? WHERE session_id=?",
                         ("Ready for the first local-data discovery run", home["session_id"]))
        [legacy] = app.research_sessions.sessions()
        assert "curing" in legacy["summary"].lower()
        with app.repository.connect() as conn:
            assert conn.execute("SELECT summary FROM research_sessions WHERE session_id=?",
                                (home["session_id"],)).fetchone()[0] == "Ready for the first local-data discovery run"
    finally:
        app.close()
