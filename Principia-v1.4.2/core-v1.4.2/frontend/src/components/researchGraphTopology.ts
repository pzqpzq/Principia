...
export function graphEdgeEndpoints(edge: Record<string, unknown>): [string, string] {
  return [String(edge.source_id ?? edge.source ?? edge.source_principle_id ?? ""),
    String(edge.target_id ?? edge.target ?? edge.target_principle_id ?? "")];
}
