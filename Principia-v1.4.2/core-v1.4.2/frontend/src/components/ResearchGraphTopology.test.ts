import { graphEdgeEndpoints } from "./researchGraphTopology";
...
it("resolves canonical evidence and Cloud relationships with the same endpoint contract", () => {
  expect(graphEdgeEndpoints({source_id:"finding:a",target_id:"law:b"})).toEqual(["finding:a","law:b"]);
  expect(graphEdgeEndpoints({source_principle_id:"principle:a",target_principle_id:"principle:b"})).toEqual(["principle:a","principle:b"]);
  expect(graphEdgeEndpoints({source:"a",target:"b"})).toEqual(["a","b"]);
});
