...
<p align="center">
  ...
  <a href="CHANGELOG.md"><img alt="Release v1.4.2" src="https://img.shields.io/badge/release-v1.4.2-111827?style=flat-square"></a>
  <a href="https://github.com/pzqpzq/Principia/blob/main/Principia-v1.4.2/core-v1.4.2/LICENSE"><img alt="MIT license" src="https://img.shields.io/badge/license-MIT-0F766E?style=flat-square"></a>
...
<p align="center">
  ...
  <a href="https://github.com/pzqpzq/Principia/tree/main/Principia-v1.4.2/core-v1.4.2/docs">Documentation</a> ·
  <a href="https://github.com/pzqpzq/Principia/tree/main/Principia-v1.4.2/core-v1.4.2/examples">Examples</a> ·
  <a href="https://github.com/pzqpzq/Principia/blob/main/Principia-v1.4.2/core-v1.4.2/qa-v1.4.2.json">Release QA</a>
</p>
...
> **Release scope:** this README documents **Principia v1.4.2**, including
> dataset-native Autonomous Scientific Discovery and all v1.4.1 literature and Cloud behavior.

## Open v1.4.2 from source

Run these commands from this source directory (Python 3.11 or 3.12 recommended).
This repository release does not require a matching PyPI release.

```bash
python3 -m venv .venv
source .venv/bin/activate
python -m pip install ".[asd,local]"
principia open --working-directory ./principia-project
```

The Python-only installed runtime serves a packaged React UI on loopback. It provides a unified research workspace, a scalable Principle graph, folder-first Local Discovery, reversible Scenario Mode, and verified Principle packages. The v1.4.2 demo distribution opens with curated public-data projects: equations, study maps, numerical evidence, and limitations are available without the original datasets or an API key. Private folders and credentials are never bundled. Existing workspaces and deleted demos are respected. See [the demo guide](docs/v1.4.2/demo-projects.md).

For a source checkout, run `python3 -m venv .venv`, then `.venv/bin/python -m pip install '.[asd,local]'` in this directory. Start with `.venv/bin/principia open --working-directory ./my-workspace`. The frontend is already built; Node.js is only needed for frontend development.

### v1.4.2 Autonomous Scientific Discovery

Select one or more local data folders, optionally state what Principia should investigate,
and choose **Discover data**. Principia keeps source bytes read-only and uses five visible
phases: **Inventory, Understand, Analyze, Challenge, Synthesize**. Dataset assets and
computed evidence are distinct from papers and quotations. Findings remain explicitly
labelled by their evidence status; automated novelty
language never exceeds `possibly novel`.

The `asd` extra adds streaming adapters for scientific tables, archives, Office documents,
FITS, HDF5/NWB, ROOT, EDF, DICOM, NetCDF, images, and supported vendor formats. Derived
state lives under `.principia/artifacts/data-discovery/`, while exports contain reports,
tests, code receipts, and figures—but never raw data. Remote reasoning or vision begins only
after one-run consent and receives bounded profiles or previews rather than complete files or
absolute paths. See [the v1.4.2 ASD guide](docs/v1.4.2/data-discovery.md).
...
atomically, while private workspace state remains separate from the shared Cloud.

Canonical reviewable records live under the repository-root
...
and the public privacy and recovery documentation.
...
1. Open **New Research** and choose the working directory from the sidebar footer.
2. Enter one research goal and press **Search**. Global paper-first retrieval starts immediately.
3. Only when needed, open **Local data & model**. Connected folders are unselected by default; explicitly select any folder whose papers should be sent to the chosen LLM. **Find papers online** stays in this workspace and downloads selected full text into a goal-named folder under `local_data/`.
4. The five highest-ranked literature Principles enter the graph, together with any validated Meta foundations. Global, Local, and Foundation results continue streaming into the hideable tray; either branch can succeed independently.
5. Add or remove map members, inspect public sources, derive virtual Principles or connections from a 2–20 item cart, and organize the automatically saved session under a one-level Project.
...
Key guides: [getting started](docs/v1.4/getting-started.md), [Local Literature Discovery](docs/v1.4/local-literature-discovery.md), [storage and portability](docs/v1.4/storage-and-portability.md), [privacy and model egress](docs/v1.4/privacy-and-security.md), [Global packages](docs/v1.4/global-packages.md), [Scenario Mode](docs/v1.4/scenarios.md), [migration and recovery](docs/v1.4/migration-and-recovery.md), and [release verification](docs/v1.4/release-verification.md).
...
| --- | --- | --- |
| **Dataset-native ASD** | Read-only multi-format inventory, typed data views, deterministic analyses, challenge checks, isolated generated Python, multimodal previews, Principle grounding, and data-derived drafts | Local scientific data becomes auditable computed evidence without being misrepresented as literature or silently mutated |
| **Product UI** | Unified Research workspace, WebGL map, Local Discovery, and Scenario tools served by the Python wheel | Installed users need no Node runtime and never fall back to mock content |
...
```bash
python -m pip install principia-ai==1.4.2
```

Add Office-document and notebook support when needed:

```bash
python -m pip install "principia-ai[asd,notebook]==1.4.2"
```
...
- [Examples](https://github.com/pzqpzq/Principia/tree/main/Principia-v1.4.2/core-v1.4.2/examples)
- [Changelog](https://github.com/pzqpzq/Principia/blob/main/Principia-v1.4/core/CHANGELOG.md)
- [Release QA](https://github.com/pzqpzq/Principia/blob/main/Principia-v1.4.2/core-v1.4.2/qa-v1.4.2.json)
...
The `principia-ai` v1.4.0 framework is released under the [MIT License](https://github.com/pzqpzq/Principia/blob/main/Principia-v1.4.2/core-v1.4.2/LICENSE).
