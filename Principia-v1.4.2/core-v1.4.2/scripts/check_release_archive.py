...
_WHEEL_NAME_RE = re.compile(
    r"^(?P<project>principia_ai|principia_admin_local)-"
    r"(?P<version>[0-9]+(?:\.[0-9]+){2}(?:[A-Za-z0-9.-]*)?)-py3-none-any\.whl$"
)
_SDIST_NAME_RE = re.compile(
    r"^(?P<project>principia_ai|principia_admin_local)-"
    r"(?P<version>[0-9]+(?:\.[0-9]+){2}(?:[A-Za-z0-9.-]*)?)\.tar\.gz$"
)
...
def _logical_path(member_name: str) -> str:
    ...
    if parts and re.fullmatch(
        r"(?:principia_ai|principia_admin_local)-"
...
def _validate_archive_pair(archives: tuple[Path, ...]) -> None:
    wheel_matches = [
        (path, match)
        for path in archives
        if (match := _WHEEL_NAME_RE.fullmatch(path.name)) is not None
    ]
    sdist_matches = [
        (path, match)
        for path in archives
        if (match := _SDIST_NAME_RE.fullmatch(path.name)) is not None
    ]
    recognized = {path for path, _ in wheel_matches + sdist_matches}
    unexpected = sorted(path.name for path in archives if path not in recognized)
    details: list[str] = []
    if len(wheel_matches) != 1:
        if not wheel_matches and len(sdist_matches) == 1:
            project = sdist_matches[0][1].group("project")
            version = sdist_matches[0][1].group("version")
            details.append(f"missing {project}-{version}-py3-none-any.whl")
        else:
            details.append("expected exactly one universal Python wheel")
    if len(sdist_matches) != 1:
        if not sdist_matches and len(wheel_matches) == 1:
            project = wheel_matches[0][1].group("project")
            version = wheel_matches[0][1].group("version")
            details.append(f"missing {project}-{version}.tar.gz")
        else:
            details.append("expected exactly one source distribution")
    if len(wheel_matches) == 1 and len(sdist_matches) == 1:
        wheel_project = wheel_matches[0][1].group("project")
        sdist_project = sdist_matches[0][1].group("project")
        wheel_version = wheel_matches[0][1].group("version")
        sdist_version = sdist_matches[0][1].group("version")
        if wheel_project != sdist_project:
            details.append(
                f"project mismatch between wheel {wheel_project} and sdist {sdist_project}"
            )
        if wheel_version != sdist_version:
            details.append(
                f"version mismatch between wheel {wheel_version} and sdist {sdist_version}"
            )
    if unexpected:
        details.append("unexpected " + ", ".join(unexpected))
    if len(archives) != 2 and not details:
        details.append("expected exactly two release archives")
    if details:
...
def _unsafe_member_reason(name: str) -> str:
    ...
    if path.name == ".DS_Store" or path.suffix in {".pyc", ".pyo"}:
        return "runtime/cache member"
    if path.name.startswith(".env") or path.suffix == ".env" or path.name in {"credentials.json", "provider-secrets.json"}:
        return "credential file member"
...
def _content_findings(member: _Member) -> list[str]:
    findings: list[str] = []
    if member.name.endswith("demo_projects/knowledge.pcg"):
        try:
            with zipfile.ZipFile(io.BytesIO(member.data)) as archive:
                if sum(item.file_size for item in archive.infolist()) > 128 * 1024**2:
                    return ["oversized public knowledge snapshot"]
                for name in archive.namelist():
                    data = archive.read(name)
                    # SQLite stores public JSON as UTF-8 amid binary page headers.
                    if name == "cloud.sqlite":
                        data = data.decode("utf-8", errors="ignore").encode()
                    findings.extend(_content_findings(_Member(name, data)))
        except (ValueError, OSError, zipfile.BadZipFile):
            findings.append("invalid public knowledge snapshot")
        return findings
    if member.name.endswith("demo_projects/public-v142.json.gz"):
        try:
            with gzip.GzipFile(fileobj=io.BytesIO(member.data)) as stream:
                expanded = stream.read(96 * 1024**2 + 1)
            if len(expanded) > 96 * 1024**2:
                return ["oversized demo bundle"]
            payload = json.loads(expanded)
            findings.extend(_content_findings(_Member(member.name + ".json", expanded)))
            for artifact in payload.get("artifacts", []):
                data = base64.b64decode(artifact["content"], validate=True)
                findings.extend(_content_findings(_Member(artifact["path"], data)))
        except (ValueError, OSError, KeyError):
            findings.append("invalid demo bundle")
        return findings
...
def verify_release_archives(paths: Iterable[Path]) -> ArchiveScanSummary:
    """Verify one same-version Principia wheel/sdist pair and return scan counts."""

    archives = tuple(sorted((Path(path).resolve() for path in paths), key=lambda item: item.name))
    _validate_archive_pair(archives)
