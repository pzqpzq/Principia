...
class SiliconFlowEmbeddingClient:
    def __init__(
        ...
        *,
        ...
        from principia.providers.token_budget import TokenBudget
        ...
        *,
        ...
        for attempt in range(self.max_retries + 1):
            ...
            try:
                ...
                try:
                    reservation = self.token_budget.reserve([{"input": missing_inputs}], 0) if self.token_budget else ""
                    response = client.post(self.base_url + "/embeddings", json=payload)
                    response.raise_for_status()
                    body = response.json()
                    if self.token_budget and reservation:
                        usage = dict(body.get("usage") or {})
                        self.token_budget.settle(reservation, {"prompt_tokens": usage.get("prompt_tokens", usage.get("total_tokens")), "completion_tokens": 0})
                finally:
                    unregister()
                ...
                last_error = RuntimeError(
                    f"Embedding HTTP {exc.response.status_code}; response body omitted."
                )
                if (
                ...
                control_check_cancelled(control_token)
                last_error = RuntimeError("Embedding transport failed.")
                if attempt >= self.max_retries:
                    break
