...
SOURCE_PROVIDER_TERMS = {
    ...
    "openreview": "https://docs.openreview.net/reference/api-v2/openapi-definition",
}
...
def normalize_source_query(name: str, query: str) -> str:
    ...
    if name in {"openalex", "crossref", "semantic_scholar", "europe_pmc", "openreview"}:
...
def _fetch_bytes(url: str, timeout: float, *, control_token: Any | None = None) -> bytes:
    ...
    fetch = getattr(control_token, "fetch_bytes", None)
    if callable(fetch):
        return fetch(url, max(0.1, float(timeout or 12.0)), verify=ssl_context,
...
def _openreview_value(content: dict[str, Any], key: str, default: Any = "") -> Any:
...
def search_openreview(
    ...
    *,
    ...
    """Search public OpenReview submission metadata and preserve venue decisions.

    OpenReview records distinguish accepted conference papers from submitted,
    rejected, and withdrawn manuscripts.  The exact public venue is retained
    for display, while only explicit accepted/publication decisions are marked
    peer reviewed.  Public PDFs remain repository copies and never imply
    acceptance by themselves.
    """

    focused = normalize_source_query("openreview", query)
    params = urllib.parse.urlencode(
        {
            "query": focused,
            "content": "all",
    ...
    data = _fetch_json(
        f"https://api2.openreview.net/notes/search?{params}",
        timeout,
        control_token=control_token,
    )
    ...
    for note in data.get("notes", []) if isinstance(data, dict) else []:
        ...
        if not isinstance(content, dict):
            continue
        title = normalize_scholarly_title(_openreview_value(content, "title", ""))
        abstract = clean_text(_openreview_value(content, "abstract", ""))
        if not title or not abstract:
            continue
        venue = clean_text(_openreview_value(content, "venue", ""))
        venue_id = clean_text(_openreview_value(content, "venueid", ""))
        venue_key = f"{venue} {venue_id}".casefold()
        ...
        ) or ("submission" in venue_key and "accepted_submission" not in venue_key)
        explicit_acceptance = bool(
            "published as a conference paper" in venue_key
            or "accepted_submission" in venue_key
            or re.search(r"\b(?:oral|spotlight|poster)\b", venue_key)
        )
        final_named_venue = bool(venue and venue.casefold() not in {"openreview", "preprint"})
        accepted = (explicit_acceptance or final_named_venue) and not negative_decision
        note_id = str(note.get("id") or note.get("forum") or "")
        pdf_path = str(_openreview_value(content, "pdf", "") or "")
        pdf_url = urllib.parse.urljoin("https://openreview.net", pdf_path) if pdf_path else ""
        if pdf_url.startswith("http://"):
            pdf_url = "https://" + pdf_url.removeprefix("http://")
        forum_url = f"https://openreview.net/forum?id={note_id}" if note_id else ""
        authors_raw = _openreview_value(content, "authors", [])
        authors: list[str] = []
        if isinstance(authors_raw, list):
            for item in authors_raw:
                if isinstance(item, dict):
                    name = clean_text(
                        item.get("fullname") or item.get("name") or item.get("value") or ""
                    )
                else:
                    name = clean_text(item)
                if name:
                    authors.append(name)
        year_source = note.get("odate") or note.get("cdate") or note.get("tcdate")
        year = None
        if isinstance(year_source, (int, float)) and year_source > 0:
            year = datetime.fromtimestamp(year_source / 1000, tz=timezone.utc).year
        displayed_venue = venue or "OpenReview"
        peer_venue = re.sub(
        ...
        works.append(
            {
                ...
                "authors": authors[:12],
                "year": year,
                "venue_or_source": displayed_venue,
                "source_type": "conference-paper" if accepted else "preprint",
                "url_or_doi": forum_url,
                "pdf_url": pdf_url,
                "abstract": abstract,
                "citation_count": None,
                "source_urls": [value for value in (forum_url, pdf_url) if value],
                "community_signals": {
                    "source": "openreview",
                    "openreview_id": note_id,
                    "venue_id": venue_id,
                    "license": str(note.get("license") or ""),
                    "is_peer_reviewed": accepted,
                    "is_preprint": not accepted,
                    "publication_type": "conference-paper" if accepted else "preprint",
                    "peer_reviewed_venue": peer_venue if accepted else "",
                },
            }
        )
    return works
