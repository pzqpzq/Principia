    ...
    def __init__(
        ...
        *,
        ...
        self.data_discovery = DataDiscoveryService(
            ...
            self.repository,
            self.search,
            outputs_root=workspace.outputs_dir,
            literature_searcher=self.local.search_papers,
        )
        self.research_sessions = ResearchSessionService(
            self.repository, self.goal_runs, self.global_cloud,
            artifacts_root=workspace.storage.artifacts_dir, outputs_root=workspace.outputs_dir,
        )
        base_digest = self.content_digest()
        self.scenarios = ScenarioService(self.repository, base_digest)
        if bundled_demos:
            from ..demo_bundle import install_bundled_demos

            install_bundled_demos(self)
    ...
    def open(
        ...
        *,
        ...
        return cls(
            ...
            bundled_demos=bundled_demos,
        )

    def close(self) -> None:
        self.data_discovery.close()
