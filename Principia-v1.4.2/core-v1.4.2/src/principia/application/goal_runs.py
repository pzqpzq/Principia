...
class ResearchGoalRunService:
    ...
    def __init__(
        ...
        self._session_projection_writer: Callable[[str], None] | None = None
        self._reconcile_interrupted_runs()

    def set_session_projection_writer(self, writer: Callable[[str], None]) -> None:
        """Install the write-side projector for durable research sessions.

        Goal runs can finish independently of API polling.  Their terminal
        memberships therefore have to be copied into the owning session by
        the worker that produced them, never by a later GET request.
        """

        self._session_projection_writer = writer
    ...
    def _run(
        ...
        if self._session_projection_writer is not None:
            try:
                self._session_projection_writer(run_id)
            except Exception as exc:  # noqa: BLE001
                self.repository.append_job_event(
                    ...
                    "session_projection_failed",
                    {
                        "run_id": run_id,
                        "category": type(exc).__name__,
                        "message": (str(exc).strip() or "Session projection failed")[:500],
                    },
                    event_id=event_id(),
                )
