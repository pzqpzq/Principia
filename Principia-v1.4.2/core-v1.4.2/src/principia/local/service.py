...
class LocalDiscoveryService:
    ...
    def provider_model_catalog(self, provider_id: str) -> dict[str, Any]:
        """Read the provider's current catalog without changing local credentials."""
        profile = self.provider_profile(provider_id)
        api_key = self.credentials.api_key(provider_id)
        if not api_key:
            return {"available": False, "models": [], "vision_models": [], "message": "Configure this provider to load its model list."}
        origins = [profile.base_url]
        if profile.base_url in SILICONFLOW_AUTHORIZED_BASE_URLS:
            origins.extend(origin for origin in SILICONFLOW_AUTHORIZED_BASE_URLS if origin != profile.base_url)
        for origin in origins:
            try:
                response = httpx.get(
                    f"{origin}/models", headers={"Authorization": f"Bearer {api_key}"},
                    timeout=httpx.Timeout(15.0, connect=10.0), follow_redirects=False,
                )
                response.raise_for_status()
                payload = response.json()
                records = payload.get("data", []) if isinstance(payload, dict) else []
                models = sorted({str(item["id"]) for item in records if isinstance(item, dict)
                                 and isinstance(item.get("id"), str) and "://" not in item["id"] and _MODEL_ID.fullmatch(item["id"])})
                if not models:
                    continue
                vision = [model for model in models if model in profile.vision_models or
                          (re.search(r"(?:-VL-|vision)", model, re.I) and not
                           re.search(r"embedding|reranker|ocr", model, re.I))]
                return {"available": True, "models": models, "vision_models": vision,
                        "checked_at": utc_now(), "message": "Current model list from the provider."}
            except (httpx.HTTPError, ValueError, TypeError):
                continue
        return {"available": False, "models": [], "vision_models": [],
                "message": "The provider model list could not be loaded. You can retry or enter a known compatible model ID."}
