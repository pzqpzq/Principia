...
def _contains_concept(text: str, term: str) -> bool:
    """Match a normalized scientific term on token/phrase boundaries.

    This is especially important for short acronyms: ``AI`` must match the
    standalone scientific acronym, never the letters inside ``materials`` or
    ``detail``.
    """

    normalized_text = " ".join(
        str(text).casefold().replace("_", " ").replace("-", " ").split()
    )
    normalized_term = " ".join(
        str(term).casefold().replace("_", " ").replace("-", " ").split()
    )
    if not normalized_term:
        return False
    phrase = re.escape(normalized_term).replace(r"\ ", r"\s+")
    return re.search(rf"(?<![a-z0-9]){phrase}(?![a-z0-9])", normalized_text) is not None
...
def _semantic_row_sort_key(
    ...
    def contains(term: str) -> bool:
        return _contains_concept(haystack, term)
...
class GlobalCloudSnapshotStore:
    ...
    def bridge_principles(
        self,
        concept_groups: list[list[str]],
        *,
        limit: int = 20,
    ) -> dict[str, Any]:
        """Traverse Work-to-Principle evidence when concepts use different vocabularies.

        This path is deterministic and remains available when a verified Cloud
        release has no complete vector index. Each retained Work must contain
        at least one term from every requested concept group.
        """

        groups = [
            list(dict.fromkeys(term.casefold().strip() for term in group if term.strip()))
            for group in concept_groups
            if any(term.strip() for term in group)
        ]
        if len(groups) < 2:
            return {"works": [], "principles": [], "match_path": "evidence_graph_bridge"}
        ...
        normalized_groups = [
            list(dict.fromkeys(normalize(term) for term in group if normalize(term)))
            for group in groups
        ]
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT work_id, title, abstract, payload_json FROM current_works "
                "WHERE status='active' ORDER BY updated_at DESC, work_id"
            ).fetchall()
        candidates: list[tuple[int, str, dict[str, Any]]] = []
        for row in rows:
            haystack = normalize(f"{row['title']} {row['abstract']}")
            hits = [
                [term for term in group if term and _contains_concept(haystack, term)]
                for group in normalized_groups
            ]
            if not all(hits):
                continue
            score = sum(len(values) for values in hits)
            candidates.append(
                (
                    -score,
                    str(row["work_id"]),
        ...
        candidates.sort(key=lambda item: (item[0], item[1]))
        works = [item[2] for item in candidates[: max(1, min(limit, 50))]]
        if not works:
            return {"works": [], "principles": [], "match_path": "evidence_graph_bridge"}
        work_ids = [str(item["work_id"]) for item in works]
        placeholders = ",".join("?" for _ in work_ids)
        with self._connect() as conn:
            links = conn.execute(
                f"SELECT DISTINCT work_id, principle_id FROM principle_work "
                f"WHERE work_id IN ({placeholders}) ORDER BY work_id, principle_id",
                work_ids,
            ).fetchall()
        work_by_id = {str(item["work_id"]): item for item in works}
        principle_provenance: dict[str, list[dict[str, Any]]] = {}
        principle_order: list[str] = []
        for row in links:
            work = work_by_id.get(str(row["work_id"]))
            if work is None:
                continue
            principle_id = str(row["principle_id"])
            principle_provenance.setdefault(principle_id, []).append(
                {
                    "work_id": work["work_id"],
                    "title": work["title"],
                    "matched_concepts": work["matched_concepts"],
                }
            )
        linked_by_work: dict[str, list[str]] = {}
        for row in links:
            linked_by_work.setdefault(str(row["work_id"]), []).append(
                str(row["principle_id"])
            )
        for work_id in work_ids:
            for principle_id in linked_by_work.get(work_id, []):
                if principle_id not in principle_order:
                    principle_order.append(principle_id)
        principles: list[dict[str, Any]] = []
        for principle_id in principle_order:
            detail = self.principle(principle_id)
            if detail is None:
                continue
            principles.append(
                {
                    **detail,
                    "match_path": "evidence_graph_bridge",
                    "bridge_provenance": principle_provenance[principle_id],
                }
            )
            if len(principles) >= max(1, min(limit, 100)):
                break
        return {
            "works": works,
            "principles": principles,
