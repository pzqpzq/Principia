...
from ..cancellation import cancellable_wait, check_cancelled, http_request
from ..domain import (
    CandidateDraftBatch,
    CandidatePrinciple,
...
class OpenAICompatibleProvider:
    def __init__(
        ...
        *,
        ...
        self.token_budget = TokenBudget.from_environment()
        ...
        *,
        ...
        for attempt in range(1, 4):
            ...
            try:
                ...
                reservation = self.token_budget.reserve(messages, max_tokens, effective_thinking_budget or 0) if self.token_budget else ""
                response = http_request(
                    self._client, "POST", self.policy.base_url.rstrip("/") + "/chat/completions",
                ...
                if self.token_budget:
                    self.token_budget.settle(reservation, usage)
                return (
                    text,
                    {
                ...
                if attempt == 3 or not retryable or isinstance(exc, httpx.ReadTimeout):
                    break
                cancellable_wait(self._retry_delay(response, attempt))
        status = response.status_code if response is not None else None
    ...
    def available_models(self) -> list[dict[str, Any]]:
        """Return provider model records without leaking credentials into receipts."""
        ...
        try:
            response = http_request(self._client, "GET",
                self.policy.base_url.rstrip("/") + "/models", transport=self.transport,
                headers={"Authorization": f"Bearer {self.api_key}"},
            )
            response.raise_for_status()
            ...
            raise ProviderRequestError(
                f"{self.policy.provider} model catalog could not be loaded",
                category="provider_unavailable",
                retryable=True,
            ) from exc
        ...
        if not isinstance(records, list):
            raise ProviderRequestError(
                f"{self.policy.provider} returned an invalid model catalog",
                category="provider_unavailable",
                retryable=True,
            )
    ...
    def _generate_typed(
        ...
        *,
        ...
        user_content: str | list[dict[str, Any]] = original_prompt
        if content_parts:
            user_content = [{"type": "text", "text": original_prompt}, *content_parts]
        messages: list[dict[str, Any]] = [
    ...
    def generate_typed(
        ...
        *,
        ...
        """Generate a schema-validated record from text and bounded image parts."""
