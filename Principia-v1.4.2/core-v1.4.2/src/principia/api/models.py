...
class LocalSourceResponse(DomainModel):
    ...
    readiness: Literal["not_profiled", "understanding", "ready", "partial", "blocked"] = (
        "not_profiled"
    )
    index_job_id: str = ""
    index_progress: float = Field(default=0, ge=0, le=1)
    warning_summary: str = ""


class DataDiscoveryCreateRequest(DomainModel):
    source_ids: list[str] = Field(min_length=1, max_length=20)
    objective: str = Field(default="", max_length=4_000)
    provider: str = Field(default="siliconflow", max_length=100)
    reasoning_model: str = Field(default="auto", max_length=200)
    vision_model: str = Field(default="auto", max_length=200)
    knowledge_scope: Literal["global", "local", "combined"] = "combined"
    prior_art: Literal["survivors", "none"] = "survivors"
    budget: Literal["fast", "balanced", "deep"] = "balanced"
    session_id: str = Field(default="", max_length=200)
    project_mode: Literal["reuse", "new", "replace"] = "reuse"
    project_title: str = Field(default="", max_length=240)
    expected_session_revision: int | None = Field(default=None, ge=1)
    egress_confirmed: bool = False


class DataDiscoveryBlueprintPatchRequest(DomainModel):
    expected_edit_revision: int = Field(ge=0)
    target_binding_ids: list[str] = Field(min_length=1, max_length=1_000)
    input_binding_ids: list[str] = Field(min_length=1, max_length=5_000)
    nuisance_binding_ids: list[str] = Field(default_factory=list, max_length=2_000)
    independent_unit_binding_ids: list[str] = Field(min_length=1, max_length=1_000)
    interpretation_summary: str | None = Field(default=None, min_length=12, max_length=4_000)
