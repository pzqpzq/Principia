...
from ..domain import JobRecord, canonical_sha256
...
def create_app(
    ...
    *,
    ...
    @router.get("/provider-profiles/{provider_id}/models")
    def provider_model_catalog(provider_id: str) -> dict[str, Any]:
        return principia.local.provider_model_catalog(provider_id)
    ...
    def research_session(session_id: str) -> dict[str, Any]:
        item = principia.research_sessions.detail(session_id, synchronize=False)
        if item is None:
            raise KeyError(session_id)
        return item

    @router.get("/research-sessions/{session_id}/runs")
    def research_session_runs(session_id: str) -> dict[str, Any]:
        item = principia.research_sessions.detail(session_id, synchronize=False)
        if item is None:
            raise KeyError(session_id)
        return {
            "items": list(item.get("data_runs") or []),
            "literature_runs": list(item.get("runs") or []),
            "canonical_session_id": str(
                item.get("canonical_session_id") or item.get("session_id") or session_id
            ),
        }

    @router.get("/research-sessions/{session_id}/workspace")
    def research_session_workspace(
        session_id: str,
        response: Response,
        request: Request,
        run_id: str = "",
        ...
        identity = next(
            (
                candidate
                for candidate in principia.research_sessions.sessions(session_id=canonical_id, include_archived=True)
                if str(candidate.get("session_id") or "") == canonical_id
            ),
            item,
        )
        runs = list(item.get("data_runs") or [])
        preferred = next(
            (
                run
                for run in runs
                if str(run.get("state") or "")
                in {"queued", "running", "pausing", "paused", "resuming"}
            ),
            None,
        ) or next(
            (
                run
                for run in runs
                if str(run.get("state") or "") in {"succeeded", "partial"}
            ),
            None,
        ) or next(
            (run for run in runs if str(run.get("state") or "") != "cancelled"),
            runs[0] if runs else None,
        )
        selected = next(
            (run for run in runs if str(run.get("study_id") or "") == run_id),
            preferred,
        )
        study_id = str((selected or {}).get("study_id") or "")
        projection = (
            principia.data_discovery.workspace_records(study_id)
            if study_id
            else {
        ...
        if study_id or item.get("kind") == "data_discovery":
            projection, graph = principia.research_sessions.workspace_graph(
                canonical_id, projection, session=item
            )
        else:
            graph = principia.research_sessions.graph(canonical_id, session=item)
        revision = canonical_sha256(
            {
                "session_revision": item.get("revision", 0),
                "graph_revision": item.get("graph_revision", 0),
                "study_id": study_id,
                "study_updated_at": (selected or {}).get("updated_at", ""),
                "record_counts": projection.get("counts", {}),
                "record_digest": canonical_sha256(projection),
            }
        )
        response.headers["ETag"] = f'"{revision}"'
        response.headers["Cache-Control"] = "private, no-cache"
        if request.headers.get("if-none-match") == f'"{revision}"':
            return Response(status_code=304, headers={"ETag": f'"{revision}"', "Cache-Control": "private, no-cache"})
        return {
            "schema_version": "principia.research-workspace/v1",
            "revision": revision,
            "project": {
                "session_id": canonical_id,
                "requested_session_id": session_id,
                "title": identity.get("display_title") or identity.get("title") or "Research",
                "summary": identity.get("summary") or "",
                "activity_summary": identity.get("activity_summary") or "",
                "source_ids": item.get("source_ids") or [],
                "source_set_digest": item.get("source_set_digest") or "",
                "redirect_session_id": item.get("redirect_session_id") or "",
                "preferred_study_id": item.get("preferred_study_id") or "",
            },
            "runs": runs,
            ...
            "graph": graph,
            "capabilities": {
                "record_projection": "canonical",
                "legacy_evidence": bool(
                    selected
                    and int(dict(selected.get("coverage") or {}).get("executed_test_count") or 0)
                    != principia.repository.data_study_counts(study_id)["tests"]
                ),
            },
        }
    ...
    def research_session_graph(
        session_id: str, response: Response, request: Request, run_id: str = ""
    ) -> Any:
        workspace = research_session_workspace(session_id, response, request, run_id)
        return workspace if isinstance(workspace, Response) else workspace["graph"]
    ...
    def search_principles(
        ...
        intent: Literal["auto", "intersection", "bridge", "either"] = "auto",
        area: str = "",
        goal_id: str = "",
        source_id: str = "",
        session_id: str = "",
        ...
        return principia.search.search_with_plan(
            q,
            scope=scope,
            intent=intent,
            area=area,
    ...
    @router.get("/local/sources/{source_id}/assets")
    def local_source_assets(
        source_id: str,
        limit: int = Query(default=100, ge=1, le=500),
        offset: int = Query(default=0, ge=0),
    ) -> dict[str, Any]:
        return principia.data_discovery.source_assets(
            source_id, limit=limit, offset=offset
        )

    @router.get("/local/sources/{source_id}/assets/{asset_id}/preview")
    def local_source_asset_preview(
        source_id: str, asset_id: str, response: Response
    ) -> dict[str, Any]:
        response.headers["Cache-Control"] = "no-store"
        response.headers["Pragma"] = "no-cache"
        return principia.data_discovery.preview(source_id, asset_id)

    @router.post("/data-discoveries", status_code=202)
    def create_data_discovery(payload: DataDiscoveryCreateRequest) -> dict[str, Any]:
        session_id = "" if payload.project_mode == "new" else payload.session_id
        home: dict[str, Any] = {}
        if payload.project_mode == "replace" and not session_id:
            raise ValueError("select the project whose current results should be replaced")
        for source_id in payload.source_ids:
            source_root = principia.repository.source_root(source_id)
            if source_root is None or not source_root.is_dir():
                raise KeyError(f"unknown or unavailable Local source: {source_id}")
        requested_digest = principia.research_sessions.source_set_digest(payload.source_ids)
        if session_id:
            selected_home = principia.research_sessions.detail(
                session_id, synchronize=False
            )
            if selected_home is None:
                raise KeyError(session_id)
            if selected_home.get("kind") != "data_discovery":
                raise ValueError("select a local-data project for this discovery")
            if payload.expected_session_revision is not None and selected_home.get("revision") != payload.expected_session_revision:
                raise ValueError("the project changed; reload it before starting discovery")
            if payload.project_mode != "replace" and str(selected_home.get("source_set_digest") or "") != requested_digest:
                session_id = ""
            else:
                session_id = str(
                    selected_home.get("canonical_session_id")
                    or selected_home.get("session_id")
                    or session_id
                )
        if not session_id:
            labels = []
            for source_id in payload.source_ids:
                source = principia.repository.source(source_id) or {}
                labels.append(str(source.get("display_name") or "Local data"))
            source_label = " + ".join(labels[:2])
            if len(labels) > 2:
                source_label += f" +{len(labels) - 2}"
            # A research home needs a memorable dataset identity.  The optional
            # objective is retained in the immutable study request and shown as
            # context, but long default objectives make unusable sidebar titles.
            title = payload.project_title.strip() or f"Discovery · {source_label}"
            home = principia.research_sessions.create_data_discovery_home(
                source_ids=payload.source_ids,
                title=title,
                provider_profile_id=payload.provider,
                model=payload.reasoning_model,
                force_new=payload.project_mode == "new",
            )
            session_id = str(home["session_id"])
        result = principia.data_discovery.create(
            source_ids=payload.source_ids,
            ...
            source_set_digest=requested_digest,
            expected_session_revision=payload.expected_session_revision if payload.project_mode != "new" else None,
            egress_confirmed=payload.egress_confirmed,
        )
        result["run_id"] = result.get("study_id")
        result["created_project"] = bool(home.get("created_project")) if home else False
        return result

    @router.get("/data-discoveries/{study_id}")
    def data_discovery(study_id: str) -> dict[str, Any]:
        return principia.data_discovery.get(study_id)

    @router.get("/data-discoveries/{study_id}/provider-attempts")
    def data_discovery_provider_attempts(study_id: str) -> dict[str, Any]:
        study = principia.data_discovery.get(study_id)
        job = dict(study.get("job") or {})
        return {
            "schema_version": "principia.provider-attempt-receipts/v1",
            "items": principia.repository.provider_attempts(str(job.get("job_id") or "")),
        }

    @router.get("/data-discoveries/{study_id}/blueprint")
    def data_discovery_blueprint(study_id: str) -> dict[str, Any]:
        return principia.data_discovery.blueprint(study_id)

    @router.patch("/data-discoveries/{study_id}/blueprint")
    def patch_data_discovery_blueprint(
        study_id: str, payload: DataDiscoveryBlueprintPatchRequest
    ) -> dict[str, Any]:
        return principia.data_discovery.patch_blueprint(
            study_id, payload.model_dump(mode="json")
        )

    @router.get("/data-discoveries/{study_id}/status")
    def data_discovery_status(study_id: str) -> dict[str, Any]:
        status = principia.repository.data_study_status(study_id)
        if status is None:
            raise KeyError(f"unknown data study: {study_id}")
        return {**status, "counts": principia.repository.data_study_counts(study_id)}

    @router.get("/data-discoveries/{study_id}/events")
    async def data_discovery_events(
        study_id: str,
        request: Request,
        ...
        if after == 0:
            try:
                after = max(0, int(request.headers.get("last-event-id", "0") or 0))
            except ValueError:
                after = 0
        if "text/event-stream" not in request.headers.get("accept", ""):
            return {"items": await asyncio.to_thread(principia.data_discovery.events, study_id, after=after)}
        if await asyncio.to_thread(principia.repository.data_study_status, study_id) is None:
            raise KeyError(f"unknown data study: {study_id}")
        ...
        async def stream() -> Any:
            ...
            while True:
                if await request.is_disconnected():
                    return
                rows = await asyncio.to_thread(principia.data_discovery.events, study_id, after=cursor)
                for item in rows:
                ...
                record = await asyncio.to_thread(principia.repository.data_study_status, study_id)
                state = str((record or {}).get("state", "interrupted"))
                if state in {"succeeded", "partial", "failed", "cancelled", "interrupted"} and not rows:
                    yield (
                        f"event: {state}\n"
                        f"data: {json.dumps({'study_id': study_id, 'state': state}, separators=(',', ':'))}\n\n"
                    )
                    return
    ...
    @router.get("/data-discoveries/{study_id}/findings")
    def data_discovery_findings(study_id: str) -> dict[str, Any]:
        return {"items": principia.data_discovery.findings(study_id)}

    @router.get("/data-discoveries/{study_id}/rules")
    def data_discovery_rules(study_id: str) -> dict[str, Any]:
        return {
            "schema_version": "principia.data-rules/v1",
            "items": principia.data_discovery.rules(study_id),
            "candidates": principia.data_discovery.rule_candidates(study_id),
            "excluded_legacy_equation_count": (
                principia.data_discovery.legacy_equation_count(study_id)
            ),
        }

    @router.get("/data-discoveries/{study_id}/programs")
    def data_discovery_programs(study_id: str) -> dict[str, Any]:
        return {
            "schema_version": "principia.scientific-program-collection/v1",
            "items": principia.data_discovery.programs(study_id),
        }

    @router.get("/data-discoveries/{study_id}/programs/{program_id}")
    def data_discovery_program(study_id: str, program_id: str) -> dict[str, Any]:
        return principia.data_discovery.program(study_id, program_id)

    @router.get("/data-discoveries/{study_id}/laws")
    def data_discovery_laws(study_id: str) -> dict[str, Any]:
        return {
            "schema_version": "principia.scientific-law-collection/v1",
            "items": principia.data_discovery.laws(study_id),
        }

    @router.get("/data-discoveries/{study_id}/laws/{law_id}")
    def data_discovery_law(study_id: str, law_id: str) -> dict[str, Any]:
        return principia.data_discovery.law(study_id, law_id)

    @router.get(
        "/data-discoveries/{study_id}/laws/{law_id}/calibrations/{calibration_id}"
    )
    def data_discovery_law_calibration(
        study_id: str, law_id: str, calibration_id: str
    ) -> dict[str, Any]:
        return principia.data_discovery.law_calibration(
            study_id, law_id, calibration_id
        )

    @router.get("/data-discoveries/{study_id}/extra-principles")
    def data_discovery_extra_principles(study_id: str) -> dict[str, Any]:
        return {
            "schema_version": "principia.data-extra-principles/v1",
            "items": principia.data_discovery.extra_principles(study_id),
        }

    @router.get("/data-discoveries/{study_id}/links")
    def data_discovery_links(study_id: str) -> dict[str, Any]:
        return principia.data_discovery.linked_principles(study_id)

    @router.get("/data-discoveries/{study_id}/extra-principles/export")
    def data_discovery_extra_principles_export(
        study_id: str,
        format: Literal["json", "md"] = Query(default="json"),
    ) -> FileResponse:
        principia.data_discovery.get(study_id)
        filename = f"extra-principles.{format}"
        path, mime = principia.data_discovery.artifact(study_id, filename)
        return FileResponse(
            path,
            media_type=mime,
            filename=filename,
    ...
    @router.get("/data-discoveries/{study_id}/findings/{finding_id}")
    def data_discovery_finding(study_id: str, finding_id: str) -> dict[str, Any]:
        return principia.data_discovery.finding(study_id, finding_id)

    @router.get("/data-discoveries/{study_id}/artifacts")
    def data_discovery_artifacts(study_id: str) -> dict[str, Any]:
        return {
            "schema_version": "principia.data-discovery-artifacts/v1",
            "items": principia.data_discovery.artifacts(study_id),
        }

    @router.get("/data-discoveries/{study_id}/artifacts/{artifact_id}")
    def data_discovery_artifact(study_id: str, artifact_id: str) -> FileResponse:
        path, mime = principia.data_discovery.artifact(study_id, artifact_id)
        return FileResponse(
            path,
            media_type=mime,
    ...
    @router.post(
        "/data-discoveries/{study_id}/findings/{finding_id}/principle-draft",
        status_code=201,
    )
    def data_discovery_principle_draft(
        study_id: str, finding_id: str
    ) -> dict[str, Any]:
        return principia.data_discovery.prepare_principle_draft(study_id, finding_id)

    @router.post("/data-derived-principle-drafts/{draft_id}/approve")
    def approve_data_derived_principle(draft_id: str) -> dict[str, Any]:
        return principia.data_discovery.decide_draft(draft_id, "approved")

    @router.post("/data-derived-principle-drafts/{draft_id}/reject")
    def reject_data_derived_principle(draft_id: str) -> dict[str, Any]:
        return principia.data_discovery.decide_draft(draft_id, "rejected")

    @router.post("/data-discoveries/{study_id}/cancel")
    def cancel_data_discovery(study_id: str) -> dict[str, Any]:
        return principia.data_discovery.cancel(study_id)

    @router.post("/data-discoveries/{study_id}/pause")
    def pause_data_discovery(study_id: str) -> dict[str, Any]:
        return principia.data_discovery.pause(study_id)

    @router.post("/data-discoveries/{study_id}/resume")
    def resume_data_discovery(study_id: str) -> dict[str, Any]:
        return principia.data_discovery.resume(study_id)
