...
DATA_DISCOVERY_MIGRATION_VERSION = "1.4.2-009"
ROOT_CAUSE_REMEDIATION_MIGRATION_VERSION = "1.4.2-010"
FORMULA_FIRST_MIGRATION_VERSION = "1.4.2-011"
SCIENTIFIC_PROGRAM_MIGRATION_VERSION = "1.4.2-012"
RULE_ENGINE_MIGRATION_VERSION = "1.4.2-013"
...
CREATE TABLE IF NOT EXISTS research_goal_runs (
    ...
    state TEXT NOT NULL,
    goal TEXT NOT NULL,
    cloud_release_id TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_research_goal_runs_updated
    ON research_goal_runs(updated_at DESC, run_id);
CREATE TABLE IF NOT EXISTS research_goal_memberships (
    ...
    content_digest TEXT NOT NULL,
    payload_json TEXT NOT NULL,
    PRIMARY KEY(run_id, membership, item_id),
...
CREATE INDEX IF NOT EXISTS idx_data_studies_updated
    ON data_studies(updated_at DESC, study_id);
CREATE TABLE IF NOT EXISTS data_assets (
    ...
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    UNIQUE(study_id, portable_uri),
    FOREIGN KEY(study_id) REFERENCES data_studies(study_id)
);
CREATE INDEX IF NOT EXISTS idx_data_assets_study
    ON data_assets(study_id, status, modality, portable_uri);
CREATE TABLE IF NOT EXISTS data_views (
    ...
    payload_json TEXT NOT NULL,
    created_at TEXT NOT NULL,
    UNIQUE(study_id, content_digest),
    FOREIGN KEY(study_id) REFERENCES data_studies(study_id)
);
CREATE TABLE IF NOT EXISTS data_alignments (
    ...
    payload_json TEXT NOT NULL,
    FOREIGN KEY(study_id) REFERENCES data_studies(study_id),
    FOREIGN KEY(source_view_id) REFERENCES data_views(view_id),
    FOREIGN KEY(target_view_id) REFERENCES data_views(view_id)
);
CREATE TABLE IF NOT EXISTS data_hypotheses (
...
CREATE INDEX IF NOT EXISTS idx_data_hypotheses_study
    ON data_hypotheses(study_id, state, hypothesis_id);
CREATE TABLE IF NOT EXISTS data_tests (
    ...
    created_at TEXT NOT NULL,
    FOREIGN KEY(study_id) REFERENCES data_studies(study_id),
    FOREIGN KEY(hypothesis_id) REFERENCES data_hypotheses(hypothesis_id)
);
CREATE INDEX IF NOT EXISTS idx_data_tests_study
    ON data_tests(study_id, state, test_id);
...
CREATE INDEX IF NOT EXISTS idx_data_findings_study
    ON data_findings(study_id, status, updated_at DESC, finding_id);
CREATE TABLE IF NOT EXISTS data_extra_principles (
    extra_principle_id TEXT PRIMARY KEY,
    study_id TEXT NOT NULL,
    status TEXT NOT NULL,
    record_kind TEXT NOT NULL,
    payload_json TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_data_extra_principles_study
    ON data_extra_principles(study_id, updated_at DESC, extra_principle_id);
CREATE TABLE IF NOT EXISTS data_evidence_links (
    ...
    payload_json TEXT NOT NULL,
    FOREIGN KEY(study_id) REFERENCES data_studies(study_id),
    FOREIGN KEY(test_id) REFERENCES data_tests(test_id),
    FOREIGN KEY(asset_id) REFERENCES data_assets(asset_id),
    FOREIGN KEY(view_id) REFERENCES data_views(view_id)
);
CREATE INDEX IF NOT EXISTS idx_data_evidence_finding
    ON data_evidence_links(study_id, finding_id, evidence_id);
CREATE TABLE IF NOT EXISTS data_principle_links (
    study_id TEXT NOT NULL,
    ...
    rationale TEXT NOT NULL,
    payload_json TEXT NOT NULL DEFAULT '{}',
    PRIMARY KEY(finding_id, principle_id, relation_type),
    FOREIGN KEY(study_id) REFERENCES data_studies(study_id),
    FOREIGN KEY(finding_id) REFERENCES data_findings(finding_id)
);
CREATE TABLE IF NOT EXISTS data_derived_principle_drafts (
...
CREATE TABLE IF NOT EXISTS data_job_units (
...
CREATE INDEX IF NOT EXISTS idx_data_job_units_state
    ON data_job_units(job_id, state, ordinal);
"""

_ROOT_CAUSE_REMEDIATION_SCHEMA_SQL = """
CREATE TABLE IF NOT EXISTS research_session_aliases (
    alias_session_id TEXT PRIMARY KEY,
    canonical_session_id TEXT NOT NULL,
    preferred_study_id TEXT NOT NULL DEFAULT '',
    source_set_digest TEXT NOT NULL,
    reason TEXT NOT NULL,
    created_at TEXT NOT NULL,
    FOREIGN KEY(alias_session_id) REFERENCES research_sessions(session_id),
    FOREIGN KEY(canonical_session_id) REFERENCES research_sessions(session_id)
);
CREATE INDEX IF NOT EXISTS idx_research_session_aliases_canonical
    ON research_session_aliases(canonical_session_id, alias_session_id);
CREATE TABLE IF NOT EXISTS data_discovery_runs (
    session_id TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_data_discovery_runs_study
    ON data_discovery_runs(study_id, session_id);
CREATE TABLE IF NOT EXISTS study_blueprints (
    study_id TEXT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS workspace_records (
    ...
    content_digest TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    PRIMARY KEY(session_id, run_id, record_id),
    FOREIGN KEY(session_id) REFERENCES research_sessions(session_id)
);
CREATE INDEX IF NOT EXISTS idx_workspace_records_visible
    ON workspace_records(session_id, run_id, visible, category, record_id);
CREATE TABLE IF NOT EXISTS workspace_edges (
    ...
    content_digest TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    PRIMARY KEY(session_id, run_id, edge_id),
    FOREIGN KEY(session_id) REFERENCES research_sessions(session_id)
);
CREATE INDEX IF NOT EXISTS idx_workspace_edges_run
    ON workspace_edges(session_id, run_id, source_id, target_id);
"""

_SCIENTIFIC_PROGRAM_SCHEMA_SQL = """
CREATE TABLE IF NOT EXISTS scientific_programs (
...
CREATE INDEX IF NOT EXISTS idx_scientific_programs_study
    ON scientific_programs(study_id, track, created_at, program_id);
CREATE TABLE IF NOT EXISTS scientific_split_manifests (
...
CREATE INDEX IF NOT EXISTS idx_scientific_split_study
    ON scientific_split_manifests(study_id, created_at, split_manifest_id);
CREATE TABLE IF NOT EXISTS scientific_transform_graphs (
    transform_graph_id TEXT PRIMARY KEY,
    study_id TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS scientific_law_families (
    law_id TEXT PRIMARY KEY,
    study_id TEXT NOT NULL,
    program_id TEXT NOT NULL,
    family_kind TEXT NOT NULL,
    evidence_tier TEXT NOT NULL,
    canonical_ast_digest TEXT NOT NULL,
    family_digest TEXT NOT NULL,
    payload_json TEXT NOT NULL,
    created_at TEXT NOT NULL,
    FOREIGN KEY(study_id) REFERENCES data_studies(study_id),
    FOREIGN KEY(program_id) REFERENCES scientific_programs(program_id)
);
CREATE INDEX IF NOT EXISTS idx_scientific_laws_study
    ON scientific_law_families(study_id, evidence_tier, created_at, law_id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_scientific_laws_ast_scope
    ON scientific_law_families(study_id, canonical_ast_digest, program_id);
CREATE TABLE IF NOT EXISTS scientific_law_calibrations (
    ...
    created_at TEXT NOT NULL,
    FOREIGN KEY(study_id) REFERENCES data_studies(study_id),
    FOREIGN KEY(law_id) REFERENCES scientific_law_families(law_id),
    FOREIGN KEY(split_manifest_id) REFERENCES scientific_split_manifests(split_manifest_id)
);
CREATE INDEX IF NOT EXISTS idx_scientific_calibrations_law
    ON scientific_law_calibrations(law_id, regime, calibration_id);
CREATE TABLE IF NOT EXISTS scientific_law_candidates (
    candidate_id TEXT PRIMARY KEY,
    study_id TEXT NOT NULL,
    program_id TEXT NOT NULL,
    law_id TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_scientific_candidates_program
    ON scientific_law_candidates(program_id, state, candidate_id);
CREATE TABLE IF NOT EXISTS scientific_law_evaluations (
    evaluation_id TEXT PRIMARY KEY,
    study_id TEXT NOT NULL,
    law_id TEXT NOT NULL,
    calibration_id TEXT NOT NULL,
    split TEXT NOT NULL,
    selection_affected INTEGER NOT NULL DEFAULT 0,
    evaluation_digest TEXT NOT NULL,
    payload_json TEXT NOT NULL,
    created_at TEXT NOT NULL,
    FOREIGN KEY(study_id) REFERENCES data_studies(study_id),
    FOREIGN KEY(law_id) REFERENCES scientific_law_families(law_id),
    FOREIGN KEY(calibration_id) REFERENCES scientific_law_calibrations(calibration_id)
);
CREATE INDEX IF NOT EXISTS idx_scientific_evaluations_law
    ON scientific_law_evaluations(law_id, split, evaluation_id);
...
CREATE INDEX IF NOT EXISTS idx_scientific_decisions_program
    ON scientific_decisions(program_id, decision_type, decision_id);
"""
...
def _migration_applied(
    db_path: Path, version: str = RULE_ENGINE_MIGRATION_VERSION
) -> bool:
    ...
    with connect_sqlite(db_path) as conn:
        ...
        if version not in {
            ...
            DATA_DISCOVERY_MIGRATION_VERSION,
            ROOT_CAUSE_REMEDIATION_MIGRATION_VERSION,
            SCIENTIFIC_PROGRAM_MIGRATION_VERSION,
            RULE_ENGINE_MIGRATION_VERSION,
        }:
            return (
                conn.execute(
        ...
        if version in {
            ...
            required.add(DATA_DISCOVERY_MIGRATION_VERSION)
        if version in {
            ROOT_CAUSE_REMEDIATION_MIGRATION_VERSION,
            SCIENTIFIC_PROGRAM_MIGRATION_VERSION,
            RULE_ENGINE_MIGRATION_VERSION,
        }:
            required.add(ROOT_CAUSE_REMEDIATION_MIGRATION_VERSION)
        if version in {
            SCIENTIFIC_PROGRAM_MIGRATION_VERSION,
            RULE_ENGINE_MIGRATION_VERSION,
        }:
            required.add(SCIENTIFIC_PROGRAM_MIGRATION_VERSION)
        if version == RULE_ENGINE_MIGRATION_VERSION:
            required.add(RULE_ENGINE_MIGRATION_VERSION)
        applied = {
            str(row[0])
            for row in conn.execute(
                f"SELECT version FROM schema_migrations WHERE version IN ({','.join('?' for _ in required)})",
        ...
        tables = {
            ...
            for row in conn.execute(
                ...
                "'candidate_foundation_links','data_studies','data_assets','data_views',"
                "'data_hypotheses','data_tests','data_findings',"
                "'data_derived_principle_drafts','data_job_units',"
                "'research_session_aliases','data_discovery_runs','study_blueprints',"
                "'workspace_records','workspace_edges','scientific_programs',"
                "'scientific_split_manifests','scientific_transform_graphs',"
                "'scientific_law_families','scientific_law_calibrations',"
                "'scientific_law_candidates','scientific_law_evaluations',"
        ...
        if version in {
            ...
            expected_tables.update(
                {
                    "scientific_programs",
                    "scientific_split_manifests",
                    "scientific_transform_graphs",
                    "scientific_law_families",
                    "scientific_law_calibrations",
                    "scientific_law_candidates",
                    "scientific_law_evaluations",
                    "scientific_decisions",
                }
            )
        return applied == required and expected_tables.issubset(tables)
...
def _ensure_root_cause_remediation_columns(conn: sqlite3.Connection) -> None:
    additions = {
    ...
    columns = {
        str(row[1]) for row in conn.execute("PRAGMA table_info(research_sessions)").fetchall()
    }
    for name, declaration in additions.items():
        if name not in columns:
            conn.execute(f"ALTER TABLE research_sessions ADD COLUMN {name} {declaration}")
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_research_sessions_source_set "
        "ON research_sessions(kind, source_set_digest, canonical_session_id, updated_at DESC)"
    )


def _ensure_cross_domain_rule_columns(conn: sqlite3.Connection) -> None:
    """Add queryable rule-engine identity without rewriting historical JSON."""

    table_additions: dict[str, dict[str, str]] = {
        "scientific_programs": {
            "engine_version": "TEXT NOT NULL DEFAULT 'legacy-formula-first'",
            "executor_id": "TEXT NOT NULL DEFAULT 'unresolved'",
            "executor_version": "TEXT NOT NULL DEFAULT ''",
            "signature_identity": "TEXT NOT NULL DEFAULT ''",
        },
        "scientific_law_families": {
            "rule_kind": "TEXT NOT NULL DEFAULT 'empirical_predictive'",
            "engine_version": "TEXT NOT NULL DEFAULT 'legacy-formula-first'",
            "executor_id": "TEXT NOT NULL DEFAULT 'unresolved'",
            "executor_version": "TEXT NOT NULL DEFAULT ''",
            "signature_identity": "TEXT NOT NULL DEFAULT ''",
            "gate_contract_version": "TEXT NOT NULL DEFAULT 'legacy-boolean-gates'",
        },
    }
    for table, additions in table_additions.items():
        columns = {
        ...
        for name, declaration in additions.items():
            if name not in columns:
                conn.execute(f"ALTER TABLE {table} ADD COLUMN {name} {declaration}")
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_scientific_laws_executor "
        "ON scientific_law_families(study_id, rule_kind, executor_id, evidence_tier)"
    )
    # An AST identifies the executable equation, but variables, validation
    # contrast, and transfer scope determine whether two results are the same
    # scientific family.  The earlier AST-only index collapsed legitimate
    # calibrations (and could fail an entire study) whenever two operators used
    # the same generic canonical form.
    conn.execute("DROP INDEX IF EXISTS idx_scientific_laws_ast_scope")
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_scientific_laws_ast_scope_lookup "
        "ON scientific_law_families(study_id, canonical_ast_digest, program_id)"
    )
    conn.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS idx_scientific_laws_signature_scope "
        "ON scientific_law_families(study_id, signature_identity, program_id) "
        "WHERE signature_identity!=''"
    )


def _backfill_root_cause_remediation(conn: sqlite3.Connection, *, now: str) -> dict[str, int]:
    """Backfill stable data-project identity without merging or deleting history."""

    data_sessions = {
        str(row[0])
        for row in conn.execute(
            "SELECT DISTINCT session_id FROM data_studies WHERE session_id!=''"
        ).fetchall()
    }
    updated = 0
    for row in conn.execute(
        "SELECT session_id, source_ids_json, title FROM research_sessions ORDER BY created_at"
    ).fetchall():
        ...
        try:
            source_ids = sorted(
                dict.fromkeys(
                    str(value).strip()
                    for value in json.loads(str(row[1] or "[]"))
                    if str(value).strip()
                )
            )
        except (TypeError, json.JSONDecodeError):
        ...
        digest = canonical_sha256(
            {"schema": "principia.data-source-set/v1", "source_ids": source_ids}
        )
        updated += int(
            conn.execute(
                "UPDATE research_sessions SET kind='data_discovery', "
                "source_set_digest=?, canonical_session_id=session_id, "
                "summary=CASE WHEN summary='' THEN ? ELSE summary END "
                "WHERE session_id=?",
                (digest, "Saved local-data discovery", session_id),
            ).rowcount
        )

    run_rows = conn.execute(
        "SELECT session_id, study_id, created_at FROM data_studies "
        "WHERE session_id!='' ORDER BY session_id, created_at, study_id"
    ).fetchall()
    sequences: dict[str, int] = {}
    linked = 0
    for session_id, study_id, created_at in run_rows:
        identifier = str(session_id)
        sequences[identifier] = sequences.get(identifier, 0) + 1
        linked += int(
            conn.execute(
                "INSERT OR IGNORE INTO data_discovery_runs(session_id, study_id, sequence, created_at) "
                "VALUES (?, ?, ?, ?)",
                (identifier, str(study_id), sequences[identifier], str(created_at or now)),
            ).rowcount
        )
    return {"data_sessions_backfilled": updated, "data_runs_linked": linked}
...
def _backfill_usability_recovery(conn: sqlite3.Connection, *, now: str) -> dict[str, int]:
    ...
    for search_id, query, target_count, state, payload_json, created_at, updated_at in rows:
        ...
        provisional_count = (
            len(logical_payload.get("results") or []) if isinstance(logical_payload, dict) else 0
        )
...
def migrate_workspace(db_path: Path, *, legacy_database_existed: bool) -> dict[str, Any]:
    ...
    if _migration_applied(db_path):
        return {
            "version": RULE_ENGINE_MIGRATION_VERSION,
            "applied": False,
    ...
    with migration_lock(db_path):
        ...
        unified_checksum = hashlib.sha256(
            (
                _SCHEMA_SQL
                + _LITERATURE_SCHEMA_SQL
                + _HUMAN_CENTERED_SCHEMA_SQL
        ...
        data_discovery_checksum = hashlib.sha256(
            (
                _SCHEMA_SQL
                + _LITERATURE_SCHEMA_SQL
                + _HUMAN_CENTERED_SCHEMA_SQL
        ...
        remediation_checksum = hashlib.sha256(
            (
                _SCHEMA_SQL
                + _LITERATURE_SCHEMA_SQL
                + _HUMAN_CENTERED_SCHEMA_SQL
        ...
        rule_engine_checksum = hashlib.sha256(
            (checksum + RULE_ENGINE_MIGRATION_VERSION).encode()
        ).hexdigest()
        ...
        try:
            ...
            for statement in _GLOBAL_CLOUD_WORKFLOW_SCHEMA_SQL.split(";"):
                if statement.strip():
                    conn.execute(statement)
            conn.execute(
                "INSERT OR IGNORE INTO schema_migrations(version, checksum, applied_at) VALUES (?, ?, ?)",
            ...
            conn.execute(
                "INSERT OR IGNORE INTO schema_migrations(version, checksum, applied_at) VALUES (?, ?, ?)",
                (UNIFIED_RESEARCH_MIGRATION_VERSION, unified_checksum, now),
            )
            for statement in _DATA_DISCOVERY_SCHEMA_SQL.split(";"):
                if statement.strip():
                    conn.execute(statement)
            conn.execute(
                "INSERT OR IGNORE INTO schema_migrations(version, checksum, applied_at) VALUES (?, ?, ?)",
                (DATA_DISCOVERY_MIGRATION_VERSION, data_discovery_checksum, now),
            )
            for statement in _ROOT_CAUSE_REMEDIATION_SCHEMA_SQL.split(";"):
                if statement.strip():
                    conn.execute(statement)
            _ensure_root_cause_remediation_columns(conn)
            remediation_counts = _backfill_root_cause_remediation(conn, now=now)
            conn.execute(
                "INSERT OR IGNORE INTO schema_migrations(version, checksum, applied_at) VALUES (?, ?, ?)",
                (ROOT_CAUSE_REMEDIATION_MIGRATION_VERSION, remediation_checksum, now),
            )
            for statement in _SCIENTIFIC_PROGRAM_SCHEMA_SQL.split(";"):
                if statement.strip():
                    conn.execute(statement)
            conn.execute(
                "INSERT OR IGNORE INTO schema_migrations(version, checksum, applied_at) VALUES (?, ?, ?)",
                (FORMULA_FIRST_MIGRATION_VERSION, checksum, now),
            )
            _ensure_cross_domain_rule_columns(conn)
            conn.execute(
                "INSERT OR IGNORE INTO schema_migrations(version, checksum, applied_at) VALUES (?, ?, ?)",
                (SCIENTIFIC_PROGRAM_MIGRATION_VERSION, checksum, now),
            )
            conn.execute(
                "INSERT OR IGNORE INTO schema_migrations(version, checksum, applied_at) VALUES (?, ?, ?)",
                (RULE_ENGINE_MIGRATION_VERSION, rule_engine_checksum, now),
            )
            conn.commit()
        except Exception:
            conn.rollback()
        ...
        legacy_receipt_path = receipt_dir / f"{MIGRATION_VERSION}.json"
        if not legacy_receipt_path.exists():
            legacy_receipt = {**receipt, "version": MIGRATION_VERSION}
            legacy_receipt_path.write_text(
                json.dumps(legacy_receipt, ensure_ascii=False, indent=2, sort_keys=True) + "\n",
                encoding="utf-8",
            )
            os.chmod(legacy_receipt_path, 0o600)
        unified_receipt_path = receipt_dir / f"{UNIFIED_RESEARCH_MIGRATION_VERSION}.json"
        if not unified_receipt_path.exists():
            unified_receipt = {
                **receipt,
                "version": UNIFIED_RESEARCH_MIGRATION_VERSION,
                "schema_checksum": unified_checksum,
            }
            unified_receipt_path.write_text(
                json.dumps(unified_receipt, ensure_ascii=False, indent=2, sort_keys=True) + "\n",
                encoding="utf-8",
            )
            os.chmod(unified_receipt_path, 0o600)
        receipt_path = receipt_dir / f"{DATA_DISCOVERY_MIGRATION_VERSION}.json"
        if not receipt_path.exists():
            data_discovery_receipt = {
                **receipt,
                "version": DATA_DISCOVERY_MIGRATION_VERSION,
                "schema_checksum": data_discovery_checksum,
            }
        ...
        remediation_receipt_path = (
            receipt_dir / f"{ROOT_CAUSE_REMEDIATION_MIGRATION_VERSION}.json"
        )
        if not remediation_receipt_path.exists():
            remediation_receipt = {
                **receipt,
                "version": ROOT_CAUSE_REMEDIATION_MIGRATION_VERSION,
                "schema_checksum": remediation_checksum,
            }
            remediation_receipt_path.write_text(
                json.dumps(remediation_receipt, ensure_ascii=False, indent=2, sort_keys=True) + "\n",
                encoding="utf-8",
            )
            os.chmod(remediation_receipt_path, 0o600)
        scientific_receipt_path = (
            receipt_dir / f"{SCIENTIFIC_PROGRAM_MIGRATION_VERSION}.json"
        )
        if not scientific_receipt_path.exists():
            scientific_receipt_path.write_text(
                json.dumps(receipt, ensure_ascii=False, indent=2, sort_keys=True) + "\n",
                encoding="utf-8",
            )
            os.chmod(scientific_receipt_path, 0o600)
        rule_engine_receipt_path = (
            receipt_dir / f"{RULE_ENGINE_MIGRATION_VERSION}.json"
        )
        if not rule_engine_receipt_path.exists():
            rule_engine_receipt_path.write_text(
                json.dumps(
                    {**receipt, "schema_checksum": rule_engine_checksum},
                    ensure_ascii=False,
                    indent=2,
                    sort_keys=True,
                )
            ...
            os.chmod(rule_engine_receipt_path, 0o600)
        return receipt
