...
class _BorrowedConnection:
    """Repository calls share a transaction without committing or closing it."""
...
class V14WorkspaceRepository:
    ...
    def connect(self) -> sqlite3.Connection:
        borrowed = getattr(self._transactions, "connection", None)
    ...
    @contextmanager
    def atomic(self):
        """Publish a bounded set of synchronous writes as one complete snapshot.

        Do not perform model calls, await, or dispatch other threads inside this
        scope. Other readers continue to see the last committed snapshot.
        """
        ...
        with self.connect() as conn:
            ...
            self._transactions.connection = _BorrowedConnection(conn)
            try:
                yield
            finally:
    ...
    def source_root(self, source_id: str) -> Path | None:
        ...
        # Portable demos retain source provenance without a local raw folder.
        # Path("") would incorrectly make the process working directory a source.
    ...
    def source(self, source_id: str) -> dict[str, Any] | None:
        with self.connect() as conn:
            ...
            latest_study = conn.execute(
                """
                SELECT s.study_id, s.state, s.phase, s.job_id, s.coverage_json,
                       j.progress, j.status_message
                FROM data_studies s
                JOIN json_each(s.source_ids_json) source_value
                JOIN v14_jobs j ON j.job_id=s.job_id
                WHERE source_value.value=?
                ORDER BY s.updated_at DESC, s.study_id DESC LIMIT 1
                """,
                (source_id,),
            ).fetchone()
            ...
            if latest_study is not None:
                asset_counts = {
                    ...
                    for item in conn.execute(
                        "SELECT status, COUNT(*) AS count FROM data_assets "
                        "WHERE study_id=? GROUP BY status",
                        (latest_study["study_id"],),
                    ).fetchall()
                }
                modality_counts = {
                    str(item["modality"]): int(item["count"])
                    for item in conn.execute(
                        "SELECT modality, COUNT(*) AS count FROM data_assets "
                        "WHERE study_id=? GROUP BY modality",
                        (latest_study["study_id"],),
                    ).fetchall()
                }
        ...
        non_analyzable = sum(
            asset_counts.get(value, 0) for value in ("metadata_only", "failed", "blocked")
        )
        readiness = "not_profiled"
        if latest_study is not None:
            if str(latest_study["state"]) in {"queued", "running"}:
                readiness = "understanding"
            elif str(latest_study["state"]) == "failed" or not asset_counts.get("analyzable", 0):
                readiness = "blocked"
            elif non_analyzable:
                readiness = "partial"
            else:
        ...
        return {
            **source_row,
            ...
            "analyzable_count": asset_counts.get("analyzable", 0),
            "skipped_count": asset_counts.get("metadata_only", 0),
            "failed_count": asset_counts.get("failed", 0),
            "oversized_count": asset_counts.get("blocked", 0),
            "modality_counts": modality_counts,
            "readiness": readiness,
            "index_job_id": str(latest_study["job_id"]) if latest_study is not None else "",
            "index_progress": float(latest_study["progress"]) if latest_study is not None else 0.0,
            "warning_summary": (
                str(latest_study["status_message"] or "") if latest_study is not None else ""
            ),
        }

    def create_data_study(self, payload: dict[str, Any]) -> None:
        now = utc_now()
        session_id = str(payload.get("session_id") or "")
        with self.connect() as conn:
            conn.execute(
                """
                INSERT INTO data_studies(
                    study_id, job_id, session_id, state, phase, source_ids_json,
                    source_digest, request_json, coverage_json, report_json,
                ...
                (
                    ...
                    payload.get("phase", "inventory"),
                    json.dumps(payload.get("source_ids") or [], separators=(",", ":")),
                    payload.get("source_digest", ""),
                    json.dumps(payload.get("request") or {}, ensure_ascii=False, sort_keys=True),
                    json.dumps(payload.get("coverage") or {}, ensure_ascii=False, sort_keys=True),
                    json.dumps(payload.get("report") or {}, ensure_ascii=False, sort_keys=True),
            ...
            if session_id:
                sequence = int(
                    conn.execute(
                        "SELECT COALESCE(MAX(sequence),0)+1 FROM data_discovery_runs "
                        "WHERE session_id=?",
                        (session_id,),
                    ).fetchone()[0]
                )
                conn.execute(
                    "INSERT INTO data_discovery_runs(session_id, study_id, sequence, created_at) "
                    "VALUES (?, ?, ?, ?)",
                    (session_id, payload["study_id"], sequence, payload.get("created_at", now)),
                )

    def data_discovery_runs(self, session_id: str) -> list[dict[str, Any]]:
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT r.sequence, r.created_at, s.study_id, s.job_id, s.state, s.phase, "
                "s.source_digest, s.request_json, s.updated_at, "
                "json_object('asset_count',json_extract(s.coverage_json,'$.asset_count'),"
                "'executed_test_count',json_extract(s.coverage_json,'$.executed_test_count'),"
                "'surviving_finding_count',json_extract(s.coverage_json,'$.surviving_finding_count'),"
                "'validated_rule_count',json_extract(s.coverage_json,'$.validated_rule_count')) AS coverage_json "
                "FROM data_discovery_runs r "
                "JOIN data_studies s ON s.study_id=r.study_id "
                "WHERE r.session_id=? ORDER BY r.sequence DESC",
                (session_id,),
            ).fetchall()
    ...
    def save_study_blueprint(self, payload: dict[str, Any]) -> None:
        now = utc_now()
        serialized = json.dumps(payload, ensure_ascii=False, sort_keys=True)
        with self.connect() as conn:
            conn.execute(
                """
                INSERT INTO study_blueprints(
                    study_id, schema_version, confidence, payload_json,
                    content_digest, created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?)
                ...
                (
                    payload["study_id"],
                    payload.get("schema_version", "principia.study-blueprint/v1"),
    ...
    def study_blueprint(self, study_id: str) -> dict[str, Any] | None:
        with self.connect() as conn:
            row = conn.execute(
    ...
    def save_scientific_program(self, program: ScientificProgram) -> None:
        with self.connect() as conn:
            conn.execute(
                """
                INSERT INTO scientific_programs(
                    program_id, study_id, track, state, program_digest,
                    payload_json, created_at, updated_at, engine_version,
                    executor_id, executor_version, signature_identity
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(program_id) DO UPDATE SET
                    state=excluded.state, program_digest=excluded.program_digest,
                    payload_json=excluded.payload_json, updated_at=excluded.updated_at,
                ...
                (
                    ...
                    program.executor_id,
                    program.executor_version,
                    program.signature_identity,
                ),
            )

    def scientific_programs(self, study_id: str) -> list[ScientificProgram]:
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT payload_json FROM scientific_programs WHERE study_id=? "
                "ORDER BY track, created_at, program_id",
                (study_id,),
            ).fetchall()
        return [ScientificProgram.model_validate_json(row[0]) for row in rows]

    def scientific_program(self, study_id: str, program_id: str) -> ScientificProgram | None:
        with self.connect() as conn:
            row = conn.execute(
                "SELECT payload_json FROM scientific_programs WHERE study_id=? AND program_id=?",
                (study_id, program_id),
            ).fetchone()
        return ScientificProgram.model_validate_json(row[0]) if row else None

    def save_split_manifest(self, manifest: SplitManifest) -> None:
        with self.connect() as conn:
            conn.execute(
                """
                INSERT OR REPLACE INTO scientific_split_manifests(
                    split_manifest_id, study_id, strategy, seed_digest,
    ...
    def scientific_split_manifest(self, split_manifest_id: str) -> SplitManifest | None:
        with self.connect() as conn:
            row = conn.execute(
                "SELECT payload_json FROM scientific_split_manifests WHERE split_manifest_id=?",
                (split_manifest_id,),
            ).fetchone()
        return SplitManifest.model_validate_json(row[0]) if row else None

    def scientific_split_manifests(self, study_id: str) -> list[SplitManifest]:
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT payload_json FROM scientific_split_manifests WHERE study_id=? "
                "ORDER BY created_at, split_manifest_id",
                (study_id,),
            ).fetchall()
        return [SplitManifest.model_validate_json(row[0]) for row in rows]

    def save_transform_graph(self, graph: TransformGraph) -> None:
        with self.connect() as conn:
            conn.execute(
                """
                INSERT OR REPLACE INTO scientific_transform_graphs(
                    transform_graph_id, study_id, graph_digest, payload_json, created_at
                ) VALUES (?, ?, ?, ?, ?)
                """,
                (
                    graph.transform_graph_id,
                    graph.study_id,
                    graph.graph_digest,
    ...
    def scientific_transform_graph(self, transform_graph_id: str) -> TransformGraph | None:
        with self.connect() as conn:
            row = conn.execute(
                "SELECT payload_json FROM scientific_transform_graphs WHERE transform_graph_id=?",
                (transform_graph_id,),
            ).fetchone()
        return TransformGraph.model_validate_json(row[0]) if row else None

    def scientific_transform_graphs(self, study_id: str) -> list[TransformGraph]:
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT payload_json FROM scientific_transform_graphs WHERE study_id=? "
                "ORDER BY created_at, transform_graph_id",
                (study_id,),
            ).fetchall()
        return [TransformGraph.model_validate_json(row[0]) for row in rows]

    def save_scientific_law_family(self, law: ScientificLawFamily) -> None:
        with self.connect() as conn:
            conn.execute(
                """
                INSERT INTO scientific_law_families(
                    law_id, study_id, program_id, family_kind, evidence_tier,
                    canonical_ast_digest, family_digest, payload_json, created_at,
                    rule_kind, engine_version, executor_id, executor_version,
                    signature_identity, gate_contract_version
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(law_id) DO UPDATE SET
                    ...
                    executor_version=excluded.executor_version,
                    signature_identity=excluded.signature_identity,
                    gate_contract_version=excluded.gate_contract_version
                """,
                (
                    law.law_id,
                    law.study_id,
                    ...
                    law.engine_version,
                    law.executor_id,
                    law.executor_version,
                    law.signature_identity,
                    law.gate_contract_version,
                ),
            )

    def scientific_law_families(self, study_id: str) -> list[ScientificLawFamily]:
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT payload_json FROM scientific_law_families WHERE study_id=? "
                "ORDER BY evidence_tier DESC, created_at, law_id",
                (study_id,),
            ).fetchall()
        return [ScientificLawFamily.model_validate_json(row[0]) for row in rows]

    def scientific_law_family(self, study_id: str, law_id: str) -> ScientificLawFamily | None:
        with self.connect() as conn:
            row = conn.execute(
                "SELECT payload_json FROM scientific_law_families WHERE study_id=? AND law_id=?",
                (study_id, law_id),
            ).fetchone()
        return ScientificLawFamily.model_validate_json(row[0]) if row else None

    def save_scientific_law_calibration(self, calibration: ScientificLawCalibration) -> None:
        with self.connect() as conn:
            conn.execute(
                """
                INSERT OR REPLACE INTO scientific_law_calibrations(
                    calibration_id, study_id, law_id, split_manifest_id, regime,
                    calibration_digest, payload_json, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    calibration.calibration_id,
                    calibration.study_id,
                    calibration.law_id,
    ...
    def scientific_law_calibrations(self, law_id: str) -> list[ScientificLawCalibration]:
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT payload_json FROM scientific_law_calibrations WHERE law_id=? "
                "ORDER BY regime, calibration_id",
                (law_id,),
            ).fetchall()
        return [ScientificLawCalibration.model_validate_json(row[0]) for row in rows]

    def scientific_law_calibration(
        self, study_id: str, law_id: str, calibration_id: str
    ) -> ScientificLawCalibration | None:
        with self.connect() as conn:
            row = conn.execute(
                "SELECT payload_json FROM scientific_law_calibrations "
                "WHERE study_id=? AND law_id=? AND calibration_id=?",
                (study_id, law_id, calibration_id),
            ).fetchone()
        return ScientificLawCalibration.model_validate_json(row[0]) if row else None

    def save_scientific_law_candidate(self, candidate: LawCandidate) -> None:
        with self.connect() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO scientific_law_candidates VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    candidate.candidate_id,
                    candidate.study_id,
    ...
    def scientific_law_candidates(self, program_id: str) -> list[LawCandidate]:
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT payload_json FROM scientific_law_candidates WHERE program_id=? "
                "ORDER BY state, candidate_id",
                (program_id,),
            ).fetchall()
        return [LawCandidate.model_validate_json(row[0]) for row in rows]

    def scientific_law_candidate_summaries(self, program_id: str, law_id: str) -> list[dict[str, Any]]:
        """Inspect one law's search without decoding every program's full ASTs."""
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT candidate_id, law_id, state, candidate_digest, "
                "json_extract(payload_json,'$.complexity') AS complexity, "
                "json_extract(payload_json,'$.development_score') AS development_score, "
                "json_extract(payload_json,'$.validation_score') AS validation_score "
                "FROM scientific_law_candidates WHERE program_id=? AND law_id=? "
                "ORDER BY state, candidate_id", (program_id, law_id)
            ).fetchall()
        return [dict(row) for row in rows]

    def save_scientific_law_evaluation(self, evaluation: LawEvaluation) -> None:
        with self.connect() as conn:
            conn.execute(
                """
                INSERT OR REPLACE INTO scientific_law_evaluations(
                    evaluation_id, study_id, law_id, calibration_id, split,
                    selection_affected, evaluation_digest, payload_json, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    ...
                    evaluation.study_id,
                    evaluation.law_id,
                    evaluation.calibration_id,
                    evaluation.split,
                    int(evaluation.selection_affected),
    ...
    def scientific_law_evaluations(self, law_id: str) -> list[LawEvaluation]:
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT payload_json FROM scientific_law_evaluations WHERE law_id=? "
                "ORDER BY created_at, evaluation_id",
                (law_id,),
            ).fetchall()
        return [LawEvaluation.model_validate_json(row[0]) for row in rows]

    def save_scientific_decision(self, decision: ScientificDecision) -> None:
        with self.connect() as conn:
            conn.execute(
                """
                INSERT OR REPLACE INTO scientific_decisions(
                    decision_id, study_id, program_id, subject_id, decision_type,
                    outcome, decision_digest, payload_json, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
    ...
    def scientific_decisions(self, program_id: str) -> list[ScientificDecision]:
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT payload_json FROM scientific_decisions WHERE program_id=? "
                "ORDER BY created_at, decision_id",
                (program_id,),
            ).fetchall()
        return [ScientificDecision.model_validate_json(row[0]) for row in rows]

    def replace_workspace_projection(
        self,
        *,
        session_id: str,
        run_id: str,
        ...
        """Atomically replace the canonical map/graph projection for one run."""

        if not session_id or not run_id:
            return
        record_ids = [str(item.get("record_id") or "") for item in records]
        edge_ids = [str(item.get("edge_id") or "") for item in edges]
        if any(not identifier for identifier in record_ids):
            raise ValueError("workspace records require non-empty record_id values")
        if any(not identifier for identifier in edge_ids):
            raise ValueError("workspace edges require non-empty edge_id values")
        if len(record_ids) != len(set(record_ids)):
            raise ValueError("workspace projection contains duplicate record identities")
        if len(edge_ids) != len(set(edge_ids)):
            raise ValueError("workspace projection contains duplicate edge identities")
        now = utc_now()
        with self.connect() as conn:
            conn.execute("BEGIN IMMEDIATE")
            try:
                conn.execute(
                    "DELETE FROM workspace_records WHERE session_id=? AND run_id=?",
                    (session_id, run_id),
                )
                conn.execute(
                    "DELETE FROM workspace_edges WHERE session_id=? AND run_id=?",
                    (session_id, run_id),
                )
                for record in records:
                    ...
                    conn.execute(
                        ...
                        INSERT INTO workspace_records(
                            session_id, run_id, record_id, record_kind, category,
                            origin, visible, payload_json, content_digest, updated_at
                        ) VALUES (?,?,?,?,?,?,1,?,?,?)
                        """,
                        (
                            session_id,
                ...
                for edge in edges:
                    conn.execute(
                        ...
                        INSERT INTO workspace_edges(
                            session_id, run_id, edge_id, source_id, target_id,
                            edge_class, payload_json, content_digest, updated_at
                        ) VALUES (?,?,?,?,?,?,?,?,?)
                        """,
                        (
                            session_id,
    ...
    def workspace_projection(self, session_id: str, run_id: str) -> dict[str, Any]:
        with self.connect() as conn:
            record_rows = conn.execute(
                "SELECT record_id, record_kind, category, origin, payload_json "
                "FROM workspace_records WHERE session_id=? AND run_id=? AND visible=1 "
                "ORDER BY category, record_id",
                (session_id, run_id),
            ).fetchall()
            edge_rows = conn.execute(
                "SELECT payload_json FROM workspace_edges WHERE session_id=? AND run_id=? "
                "ORDER BY edge_id",
                (session_id, run_id),
            ).fetchall()
        records = [
        ...
        return {
            "records": records,
            "edges": [json.loads(str(row[0])) for row in edge_rows],
        }

    def update_data_study(self, study_id: str, **changes: Any) -> dict[str, Any]:
        permitted = {
            "state": "state",
            "phase": "phase",
        ...
        values: list[Any] = []
        for key, column in permitted.items():
            if key not in changes:
                continue
            value = changes[key]
            if key in {"coverage", "report"}:
        ...
        if assignments:
            ...
            with self.connect() as conn:
                changed = conn.execute(
                    f"UPDATE data_studies SET {', '.join(assignments)} WHERE study_id=?",
                    (*values, study_id),
                ).rowcount
            if not changed:
                raise KeyError(f"unknown data study: {study_id}")
        result = self.data_study(study_id)
        if result is None:
            raise KeyError(f"unknown data study: {study_id}")
        return result

    def data_study_counts(self, study_id: str) -> dict[str, int]:
        with self.connect() as conn:
            row = conn.execute(
                "SELECT (SELECT count(*) FROM data_tests WHERE study_id=?) AS tests, "
                "count(*) AS findings, coalesce(sum(status='supported_candidate'),0) AS supported_findings "
                "FROM data_findings WHERE study_id=?", (study_id, study_id)
            ).fetchone()
        return {key: int(row[key]) for key in ("tests", "findings", "supported_findings")}

    def data_study_status(self, study_id: str) -> dict[str, Any] | None:
        # Keep live progress independent of the large report/checkpoint payloads.
        # Study timestamps change at phase boundaries; job timestamps also carry
        # provider heartbeats between boundaries.
        with self.connect() as conn:
            row = conn.execute(
                "SELECT d.study_id, d.job_id, d.session_id, d.state, d.phase, d.created_at, d.updated_at, "
                "j.state AS job_state, j.stage, j.progress, j.completed_units, j.total_units, "
                "j.last_activity_at, j.status_message, j.retry_after_seconds "
        ...
        if row is None:
            return None
        result = dict(row)
        job = {name: result.pop(name) for name in (
            "stage", "progress", "completed_units", "total_units", "last_activity_at",
            "status_message", "retry_after_seconds",
        )}
        job["state"] = result.pop("job_state")
        job["job_id"] = result["job_id"]
        result["job"] = job
        return result

    def data_study(self, study_id: str) -> dict[str, Any] | None:
        with self.connect() as conn:
            row = conn.execute(
    ...
    def latest_data_study_for_session(self, session_id: str) -> dict[str, Any] | None:
        if not session_id:
            return None
        with self.connect() as conn:
            row = conn.execute(
                "SELECT r.study_id FROM data_discovery_runs r "
                "JOIN data_studies d ON d.study_id=r.study_id "
                "WHERE r.session_id=? ORDER BY "
                "r.sequence DESC LIMIT 1",
                (session_id,),
            ).fetchone()
        return self.data_study(str(row[0])) if row is not None else None

    def update_data_session_state(self, session_id: str, state: str) -> None:
        if not session_id:
            return
        with self.connect() as conn:
            conn.execute(
                "UPDATE research_sessions SET state=?, revision=revision+1, updated_at=? "
                "WHERE session_id=?",
                (state, utc_now(), session_id),
            )

    def save_data_asset(self, asset: DataAsset) -> None:
        payload = asset.model_dump(mode="json")
        with self.connect() as conn:
            conn.execute(
                """
                INSERT INTO data_assets(
                    asset_id, study_id, source_id, portable_uri, byte_sha256,
                    byte_size, format, role, modality, adapter, adapter_version,
    ...
    def data_assets(
        ...
        *,
        ...
        if not study_id and not source_id:
            raise ValueError("study_id or source_id is required")
        ...
        with self.connect() as conn:
            if study_id:
                ...
                row = conn.execute(
                    """
                    SELECT s.study_id FROM data_studies s
                    JOIN json_each(s.source_ids_json) value
                    WHERE value.value=? ORDER BY s.updated_at DESC LIMIT 1
                    """,
                    (source_id,),
                ).fetchone()
                target_study = str(row[0]) if row else ""
            if not target_study:
                return {"items": [], "total": 0, "study_id": ""}
            total = int(
                conn.execute(
                    "SELECT COUNT(*) FROM data_assets WHERE study_id=?", (target_study,)
                ).fetchone()[0]
            )
            rows = conn.execute(
                "SELECT payload_json FROM data_assets WHERE study_id=? "
                "ORDER BY portable_uri LIMIT ? OFFSET ?",
                (target_study, resolved_limit, resolved_offset),
            ).fetchall()
        return {
            "items": [DataAsset.model_validate_json(row[0]).model_dump(mode="json") for row in rows],
            "total": total,
            "study_id": target_study,
            "next_offset": resolved_offset + len(rows) if resolved_offset + len(rows) < total else None,
        }

    def data_asset(self, asset_id: str) -> DataAsset | None:
        with self.connect() as conn:
            row = conn.execute(
                "SELECT payload_json FROM data_assets WHERE asset_id=?", (asset_id,)
            ).fetchone()
        return DataAsset.model_validate_json(row[0]) if row else None

    def save_data_view(self, view: DataView) -> None:
        with self.connect() as conn:
            conn.execute(
                """
                INSERT OR REPLACE INTO data_views(
                    view_id, study_id, kind, content_digest, payload_json, created_at
                ) VALUES (?, ?, ?, ?, ?, ?)
                """,
                (
                    view.view_id,
    ...
    def data_views(self, study_id: str) -> list[DataView]:
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT payload_json FROM data_views WHERE study_id=? ORDER BY view_id",
                (study_id,),
            ).fetchall()
        return [DataView.model_validate_json(row[0]) for row in rows]

    def save_data_alignment(self, alignment: DatasetAlignment) -> None:
        with self.connect() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO data_alignments VALUES (?, ?, ?, ?, ?, ?, ?)",
                (
                    alignment.alignment_id,
                    alignment.study_id,
                    alignment.source_view_id,
                    alignment.target_view_id,
                    alignment.relation,
    ...
    def data_alignments(self, study_id: str) -> list[DatasetAlignment]:
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT payload_json FROM data_alignments WHERE study_id=? "
                "ORDER BY alignment_id",
                (study_id,),
            ).fetchall()
        return [DatasetAlignment.model_validate_json(row[0]) for row in rows]

    def save_data_job_unit(self, payload: dict[str, Any]) -> None:
        now = utc_now()
        with self.connect() as conn:
            conn.execute(
                """
                INSERT INTO data_job_units(
                    unit_id, study_id, job_id, unit_kind, ordinal, state,
                    attempt_count, input_digest, payload_json, result_json,
    ...
    def save_data_hypothesis(self, hypothesis: DataHypothesis) -> None:
        with self.connect() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO data_hypotheses VALUES (?, ?, ?, ?, ?, ?)",
                (
    ...
    def data_hypotheses(self, study_id: str) -> list[DataHypothesis]:
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT payload_json FROM data_hypotheses WHERE study_id=? "
                "ORDER BY created_at, hypothesis_id",
                (study_id,),
            ).fetchall()
        return [DataHypothesis.model_validate_json(row[0]) for row in rows]

    def save_data_test(self, payload: dict[str, Any]) -> None:
        with self.connect() as conn:
            conn.execute(
                """
                INSERT OR REPLACE INTO data_tests(
                    test_id, study_id, hypothesis_id, plan_id, state,
                    plan_digest, result_digest, payload_json, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    payload["test_id"],
    ...
    def save_data_finding(self, finding: DataFinding) -> None:
        with self.connect() as conn:
            conn.execute(
                """
                INSERT INTO data_findings(
                    finding_id, study_id, status, novelty_status, record_kind,
                    payload_json, created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(finding_id) DO UPDATE SET
                    status=excluded.status, novelty_status=excluded.novelty_status,
                    payload_json=excluded.payload_json, updated_at=excluded.updated_at
                """,
                (
                    finding.finding_id,
    ...
    def replace_data_principle_links(
        self, study_id: str, findings: list[DataFinding]
    ) -> None:
        """Persist the finding-to-Principle lineage rendered by the graph."""

        with self.connect() as conn:
            conn.execute("DELETE FROM data_principle_links WHERE study_id=?", (study_id,))
            for finding in findings:
                for principle_id in dict.fromkeys(finding.principle_ids):
                    payload = {
                        "schema_version": "principia.data-principle-link/v1",
                        "study_id": study_id,
                        "finding_id": finding.finding_id,
                        "principle_id": principle_id,
                        "relation_type": "explained_by",
                        "rationale": finding.mechanism[:2_000],
                        "principle_chain": finding.principle_chain,
                    }
                    conn.execute(
                        """
                        INSERT INTO data_principle_links(
                            study_id, finding_id, principle_id, relation_type,
                        ...
                        (
                            ...
                            finding.finding_id,
                            principle_id,
                            finding.mechanism[:2_000],
                            json.dumps(payload, ensure_ascii=False, sort_keys=True),
                        ),
                    )

    def data_principle_links(self, study_id: str) -> list[dict[str, Any]]:
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT payload_json FROM data_principle_links WHERE study_id=? "
                "ORDER BY finding_id, principle_id",
                (study_id,),
            ).fetchall()
        return [json.loads(row[0]) for row in rows]

    def save_data_extra_principle(self, principle: DataExtraPrinciple) -> None:
        with self.connect() as conn:
            conn.execute(
                """
                INSERT INTO data_extra_principles(
                    extra_principle_id, study_id, status, record_kind,
                    payload_json, created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(extra_principle_id) DO UPDATE SET
                    status=excluded.status, payload_json=excluded.payload_json,
                    updated_at=excluded.updated_at
                """,
                (
                    principle.extra_principle_id,
                    principle.study_id,
                    principle.status,
                    principle.record_kind,
                    principle.model_dump_json(),
                    principle.created_at,
                    principle.updated_at,
                ),
            )

    def data_extra_principles(self, study_id: str) -> list[DataExtraPrinciple]:
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT payload_json FROM data_extra_principles WHERE study_id=? "
                "ORDER BY updated_at DESC, extra_principle_id",
                (study_id,),
            ).fetchall()
        return [DataExtraPrinciple.model_validate_json(row[0]) for row in rows]

    def seed_data_discovery_graph(
        self,
        session_id: str,
        findings: list[DataFinding],
        principles: list[dict[str, Any]],
        extra_principles: list[DataExtraPrinciple] | None = None,
        *,
        used_principle_ids: list[str] | None = None,
    ) -> dict[str, Any]:
        """Add surviving findings and Principles used by the study.

        The session graph remains a mixed scientific-record surface: data
        findings keep their permanent ``discovery_finding`` record kind and
        established Principles remain ordinary records. A planning Principle
        may remain visible after its hypothesis is refuted, preserving the
        scientific lineage; merely retrieved candidates are not supplied in
        ``used_principle_ids``. Existing user graph positions and visibility
        choices are never overwritten.
        """

        if not session_id:
            return {"state": "not_requested", "inserted": 0}
        now = utc_now()
        survivors = sorted(
            (
                item
        ...
        allowed_principles = {
            ...
            for identifier in finding.principle_ids
        }
        allowed_principles.update(
            identifier for identifier in (used_principle_ids or []) if identifier
        )
        principle_rows = [
            item
            for item in principles
            if (
                str(item.get("principle_id") or item.get("id") or "")
                in allowed_principles
                or str(item.get("principle_class") or "") == "meta"
            )
        ]
        extras = extra_principles or []
        with self.connect() as conn:
            if (
                conn.execute(
                ...
                return {"state": "session_unavailable", "inserted": 0}
            top_z = int(
                conn.execute(
                    "SELECT COALESCE(MAX(z_index), -1) FROM research_graph_items "
                    "WHERE session_id=?",
                    (session_id,),
                ).fetchone()[0]
            )
            ...
            for row in conn.execute(
                "SELECT principle_id FROM research_graph_items "
                "WHERE session_id=? AND origin='data_discovery_explanation'",
                (session_id,),
            ).fetchall():
                identifier = str(row[0])
                if identifier and identifier not in valid_principle_ids:
                    cursor = conn.execute(
                        "DELETE FROM research_graph_items "
                        "WHERE session_id=? AND principle_id=? "
                        "AND origin='data_discovery_explanation'",
                        (session_id, identifier),
                    )
                    changed += int(cursor.rowcount > 0)
            for index, finding in enumerate(survivors):
                        ...
                        "relations": [
                            {
                                ...
                                "target": principle_id,
                                "edge_class": "computed_explanation",
                                "relation_type": "interpreted through",
                                "rationale": (
                                    "The established Principle was retrieved to explain a "
                                    "computed candidate; the edge does not establish causality."
                                ),
                            }
                ...
                cursor = conn.execute(
                    ...
                    ) VALUES (?, ?, 'discovery_finding', 'data_discovery', 1,
                              ?, ?, 'data_discovery_seed_v1', ?, ?, ?)
                    """,
                    (
                        session_id,
                        finding.finding_id,
                ...
                cursor = conn.execute(
                    "UPDATE research_graph_items SET payload_json=?, updated_at=? "
                    "WHERE session_id=? AND principle_id=? "
                    "AND origin='data_discovery'",
                    (
                        json.dumps(payload, ensure_ascii=False, sort_keys=True),
                        now,
                        session_id,
                ...
                changed += int(cursor.rowcount > 0)
            for index, principle in enumerate(principle_rows):
                identifier = str(
                    principle.get("principle_id") or principle.get("id") or ""
                )
                record_kind = (
                    "meta_principle"
                    if str(principle.get("principle_class") or "") == "meta"
                    else "ordinary"
                )
                cursor = conn.execute(
                    """
                    INSERT OR IGNORE INTO research_graph_items(
                        session_id, principle_id, record_kind, origin, visible,
                        x, y, position_source, z_index, payload_json, updated_at
                    ) VALUES (?, ?, ?, 'data_discovery_explanation', 1,
                              ?, ?, 'data_discovery_seed_v1', ?, ?, ?)
                    """,
                    (
                        session_id,
                        ...
                        360.0 + index * 56.0,
                        -180.0 + (index % 3) * 180.0,
                        top_z + len(survivors) + index + 1,
                        json.dumps(principle, ensure_ascii=False, sort_keys=True),
                        now,
                    ),
                )
                inserted += int(cursor.rowcount > 0)
            for index, principle in enumerate(extras):
                payload = principle.model_dump(mode="json")
                payload.update(
                    {
                        "id": principle.extra_principle_id,
                        "area": "data-discovery",
                        "relations": [
                            {
                                "source": finding_id,
                                "target": principle.extra_principle_id,
                                "edge_class": "provisional_explanation",
                                "relation_type": "motivates extra principle",
                                "rationale": principle.explanatory_gap,
                            }
                            for finding_id in principle.related_finding_ids
                        ],
                    }
                )
                cursor = conn.execute(
                    ...
                    INSERT OR IGNORE INTO research_graph_items(
                        session_id, principle_id, record_kind, origin, visible,
                        x, y, position_source, z_index, payload_json, updated_at
                    ) VALUES (?, ?, 'extra_principle', 'research_gap_synthesis', 1,
                              ?, ?, 'data_discovery_seed_v1', ?, ?, ?)
                    """,
                    (
                        session_id,
                        ...
                        560.0 + index * 64.0,
                        -160.0 + (index % 3) * 160.0,
                        top_z + len(survivors) + len(principle_rows) + index + 1,
                        json.dumps(payload, ensure_ascii=False, sort_keys=True),
                        now,
                    ),
                )
            ...
            if inserted or changed:
                conn.execute(
                    "UPDATE research_sessions SET graph_revision=graph_revision+1, "
                    "updated_at=? WHERE session_id=?",
                    (now, session_id),
                )
        return {
            "state": "seeded" if inserted else "already_present",
            "inserted": inserted,
            "refreshed": changed,
            "finding_count": len(survivors),
            "principle_count": len(principle_rows),
            "extra_principle_count": len(extras),
        }

    def data_findings(self, study_id: str) -> list[DataFinding]:
        with self.connect() as conn:
            rows = conn.execute(
                "SELECT payload_json FROM data_findings WHERE study_id=? "
                "ORDER BY updated_at DESC, finding_id",
                (study_id,),
            ).fetchall()
        return [DataFinding.model_validate_json(row[0]) for row in rows]

    def data_tests(self, study_id: str) -> list[dict[str, Any]]:
        """Return immutable executed-test receipts for a dataset study."""

        with self.connect() as conn:
            rows = conn.execute(
                "SELECT payload_json FROM data_tests WHERE study_id=? "
                "ORDER BY created_at, test_id",
                (study_id,),
            ).fetchall()
        retained = {
            str(payload.get("test_id") or ""): payload
            for row in rows
            if (payload := json.loads(row[0])).get("test_id")
        }

        # Early v1.4.2 builds used globally content-derived test IDs. Re-running
        # a source could therefore move a receipt row to the later study even
        # though the first study's immutable export remained intact. Read that
        # export as a compatibility fallback so historical projects stay fully
        # reopenable; new records use study-scoped IDs and never need it.
        output_path = (
            self.db_path.parent.parent.parent / "outputs" / study_id / "tests.json"
        )
        try:
            exported = json.loads(output_path.read_text(encoding="utf-8"))
            exported_items = exported.get("items") if isinstance(exported, dict) else []
            for item in exported_items if isinstance(exported_items, list) else []:
                if not isinstance(item, dict):
                    continue
                test_id = str(item.get("test_id") or "")
                exported_study = str(item.get("study_id") or study_id)
                if test_id and exported_study == study_id:
                    retained.setdefault(test_id, item)
        except (OSError, ValueError, TypeError):
            pass
        return sorted(
            retained.values(),
            key=lambda item: (str(item.get("created_at") or ""), str(item.get("test_id") or "")),
        )

    def data_finding(self, study_id: str, finding_id: str) -> dict[str, Any] | None:
        with self.connect() as conn:
            row = conn.execute(
                "SELECT payload_json FROM data_findings WHERE study_id=? AND finding_id=?",
                (study_id, finding_id),
            ).fetchone()
            if row is None:
                return None
            evidence = conn.execute(
                "SELECT payload_json FROM data_evidence_links "
                "WHERE study_id=? AND finding_id=? ORDER BY evidence_id",
                (study_id, finding_id),
            ).fetchall()
            draft = conn.execute(
                "SELECT payload_json FROM data_derived_principle_drafts WHERE finding_id=?",
                (finding_id,),
            ).fetchone()
            finding = DataFinding.model_validate_json(row[0])
            tests = []
            if finding.test_ids:
                placeholders = ",".join("?" for _ in finding.test_ids)
                tests = conn.execute(
                    f"SELECT payload_json FROM data_tests WHERE study_id=? "
                    f"AND test_id IN ({placeholders}) ORDER BY test_id",
                    (study_id, *finding.test_ids),
                ).fetchall()
        test_payloads = {
            str(payload.get("test_id") or ""): payload
            for item in tests
            if (payload := json.loads(item[0])).get("test_id")
        }
        if len(test_payloads) < len(finding.test_ids):
            # Historical v1.4.2 receipts may live only in the immutable study
            # export after a later rerun reused an early content-derived ID.
            # Reuse the same compatibility path as data_tests() so the finding
            # inspector never claims that an exported executed test is absent.
            for payload in self.data_tests(study_id):
                identifier = str(payload.get("test_id") or "")
                if identifier in finding.test_ids:
                    test_payloads.setdefault(identifier, payload)
        return {
            **finding.model_dump(mode="json"),
            "evidence": [json.loads(item[0]) for item in evidence],
            "tests": [
                test_payloads[identifier]
                for identifier in finding.test_ids
                if identifier in test_payloads
            ],
            "principle_draft": json.loads(draft[0]) if draft else None,
        }

    def save_computed_evidence(self, evidence: ComputedEvidenceAnchor) -> None:
        with self.connect() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO data_evidence_links VALUES (?, ?, ?, ?, ?, ?, ?)",
                (
    ...
    def save_data_derived_draft(self, draft: DataDerivedPrincipleDraft) -> None:
        with self.connect() as conn:
            conn.execute(
                """
                INSERT INTO data_derived_principle_drafts(
                    draft_id, study_id, finding_id, state, record_kind,
                    payload_json, created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ...
    def data_derived_draft(self, draft_id: str) -> DataDerivedPrincipleDraft | None:
        with self.connect() as conn:
            row = conn.execute(
                "SELECT payload_json FROM data_derived_principle_drafts WHERE draft_id=?",
                (draft_id,),
            ).fetchone()
        return DataDerivedPrincipleDraft.model_validate_json(row[0]) if row else None

    def decide_data_derived_draft(
        self, draft_id: str, state: str
    ) -> DataDerivedPrincipleDraft:
        if state not in {"approved", "rejected"}:
            raise ValueError("data-derived Principle drafts may only be approved or rejected")
        draft = self.data_derived_draft(draft_id)
        if draft is None:
            raise KeyError(f"unknown data-derived Principle draft: {draft_id}")
        updated = draft.model_copy(update={"state": state, "updated_at": utc_now()})
        self.save_data_derived_draft(updated)
        return updated
    ...
    def provider_attempts(self, job_id: str) -> list[dict[str, Any]]:
        """Return the centralized, already-redacted remote-attempt ledger."""

        with self.connect() as conn:
            rows = conn.execute(
                "SELECT payload_json FROM provider_attempts WHERE job_id=? "
    ...
    def interrupt_orphaned_jobs(self) -> int:
        ...
        with self.connect() as conn:
            rows = conn.execute(
                ...
                "WHERE state IN ('queued', 'running', 'cancelling', 'pausing', 'paused', 'resuming')"
            ).fetchall()
            for row in rows:
                ...
                if job.kind == "data_discovery":
                    conn.execute(
                        "UPDATE research_sessions SET state='interrupted', revision=revision+1, updated_at=? "
                        "WHERE session_id IN (SELECT session_id FROM data_studies WHERE job_id=?)",
                        (now, job.job_id),
                    )
                    conn.execute("UPDATE data_studies SET state='interrupted', updated_at=? WHERE job_id=?", (now, job.job_id))
