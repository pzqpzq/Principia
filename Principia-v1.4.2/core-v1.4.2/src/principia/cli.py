...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="principia", description="Principia v1.4.2 framework CLI")
    parser.add_argument(
        "--workspace",
        "-w",
        default=None,
    ...
    storage_command = sub.add_parser("storage", help="Inspect or reclaim database space without deleting results.")
    storage_command.add_argument("action", choices=("inspect", "compact"))
    storage_command.add_argument("--working-directory", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "storage":
        from .storage_maintenance import compact_database, database_usage

        database = Path(args.working_directory).expanduser().resolve() / "workspace/.principia/principia.sqlite"
        try:
            print_json((compact_database if args.action == "compact" else database_usage)(database))
...
def _dispatch_v14(
    ...
    *,
    ...
    product = Principia.open(
        ...
        bundled_demos=args.command == "open",
    )
