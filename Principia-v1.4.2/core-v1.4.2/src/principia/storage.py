...
class WorkspaceStorage:
    def __init__(self, root: Path) -> None:
        ...
        legacy_database_existed = self.db_path.exists() and self.db_path.stat().st_size > 0
        self.artifacts_dir = self.meta_dir / "artifacts"
        self._ensure_layout()
        self._init_db()
        from .persistence.migrations import migrate_workspace

        self.v14_migration = migrate_workspace(
            self.db_path, legacy_database_existed=legacy_database_existed
        )
    ...
    def _init_db(self) -> None:
        with self.connect() as conn:
            # Reclaim deleted pages in new workspaces without rebuilding old DBs.
