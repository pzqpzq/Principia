...
def compact_contributions(input_dir: Path, out_dir: Path) -> dict[str, Any]:
    ...
    for path in sorted(input_dir.glob("*.json")):
        try:
            ...
            continue
        if data.get("work_records"):
            accepted.append({"path": str(path), "contribution_id": data.get("contribution_id"), "works": len(data.get("work_records") or [])})
        else:
            rejected.append({"path": str(path), "error": "no work records"})
    report = {"accepted": accepted, "rejected": rejected, "created_at": utc_now()}
