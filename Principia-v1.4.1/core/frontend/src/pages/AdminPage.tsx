...
export function AdminPage() {
  ...
  return <div className="page admin-page">
    ...
    {tab === "Discover" ? <section className="admin-discover">
      ...
      {!discovering && paperRows.length ? <><div className="paper-selection-bar"><button onClick={() => setSelectedWorks(Array.from(new Set([...selectedWorks, ...visiblePaperIds])))}>Select all shown</button><button onClick={() => setSelectedWorks([])} disabled={!selectedWorks.length}>Clear</button><span>{selectedWorks.length} selected</span></div><div className="admin-paper-table">{paperRows.map((paper) => { const id = text(paper.work_id); return <article className={selectedWorks.includes(id) ? "selected" : ""} key={id}><label className="inline-check"><input type="checkbox" checked={selectedWorks.includes(id)} onChange={(event) => setSelectedWorks((current) => event.target.checked ? Array.from(new Set([...current, id])) : current.filter((value) => value !== id))} /><span /></label><div><strong>{text(paper.title)}</strong><small>{text(paper.venue) || "Venue unknown"} · {String(paper.year ?? "year unknown")} · Full text: {String(paper.availability_status)} · Cloud: {String(paper.cloud_presence)}</small>{text(paper.abstract) ? <p>{text(paper.abstract).slice(0, 280)}</p> : null}</div></article>; })}</div></> : null}
