...
export function AdminPage() {
  ...
  return <div className="page admin-page">
    ...
    {tab === "Discover" ? <section className="admin-discover">
      ...
      {campaignId ? <div className={`discovery-status ${discovering ? "running" : "ready"}`} role="status"><span className="branch-state" aria-hidden="true" /><div><strong>{discovering ? "Retrieving and enriching public papers…" : `${extractablePaperIds.length} extractable papers ready`}</strong><small>{discovering ? "Results will appear here as soon as discovery finishes." : `${metadataOnlyPapers} metadata-only result${metadataOnlyPapers === 1 ? " is" : "s are"} shown for reference and cannot be selected. Choose at least four full-text papers.`}</small></div></div> : null}
      ...
      {!discovering && paperRows.length ? <><div className="paper-selection-bar"><button disabled={!extractablePaperIds.length} onClick={() => setSelectedWorks(Array.from(new Set([...selectedWorks, ...extractablePaperIds])))}>Select all extractable ({extractablePaperIds.length})</button><button onClick={() => setSelectedWorks([])} disabled={!selectedWorks.length}>Clear</button><span>{selectedWorks.length} selected</span></div><div className="admin-paper-table">{paperRows.map((paper) => { const id = text(paper.work_id); const extractable = paper.availability_status === "available"; return <article className={selectedWorks.includes(id) ? "selected" : ""} key={id}><label className="inline-check"><input type="checkbox" disabled={!extractable} checked={selectedWorks.includes(id)} aria-label={extractable ? `Select ${text(paper.title)}` : `${text(paper.title)} has no verified full text`} onChange={(event) => setSelectedWorks((current) => event.target.checked ? Array.from(new Set([...current, id])) : current.filter((value) => value !== id))} /><span /></label><div><strong>{text(paper.title)}</strong><small>{text(paper.venue) || "Venue unknown"} · {String(paper.year ?? "year unknown")} · {extractable ? "Full text ready" : "Metadata only — cannot extract"} · Cloud: {String(paper.cloud_presence)}</small>{text(paper.abstract) ? <p>{text(paper.abstract).slice(0, 280)}</p> : null}</div></article>; })}</div></> : null}
