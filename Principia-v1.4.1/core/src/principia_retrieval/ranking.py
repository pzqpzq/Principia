...
def final_select(
    ...
    if len(eligible) < target_count:
        eligible_keys = {selection_key(item) for item in eligible}
        for work in works:
            if normalize_relation(work.get("relation_label")) == "out_of_scope" and clean_text(
                work.get("reject_reason") or ""
            ):
                continue
            work_key = selection_key(work)
            if work_key in eligible_keys:
                continue
            eligible.append(work)
            eligible_keys.add(work_key)
            if len(eligible) >= target_count:
                break
    selected: list[dict[str, Any]] = []
    remaining = list(eligible)
    # Tokenization dominates final selection for realistic abstracts.  Cache it
    # exactly once per record and maintain each remaining record's maximum
    # similarity to the selected set.  This preserves the previous greedy
    ...
    max_similarities = {id(work): 0.0 for work in remaining}
    while remaining and len(selected) < target_count:
        ...
        chosen_tokens, chosen_venue = diversity_features[id(chosen)]
        for work in remaining:
            work_tokens, work_venue = diversity_features[id(work)]
            similarity = _diversity_similarity(
                work_tokens,
                work_venue,
                chosen_tokens,
                chosen_venue,
            )
            max_similarities[id(work)] = max(max_similarities[id(work)], similarity)
...
def diversity_novelty(work: dict[str, Any], selected: list[dict[str, Any]]) -> float:
    ...
    for existing in selected:
        ...
        similarities.append(
            _diversity_similarity(
                tokens,
                venue,
                other,
                normalize_text(existing.get("venue_or_source") or ""),
            )
        )
    return 1.0 - max(similarities, default=0.0)


def _diversity_similarity(
    left_tokens: set[str],
    left_venue: str,
    ...
    venue_overlap = 0.1 if left_venue and left_venue == right_venue else 0.0
    return min(1.0, lexical + venue_overlap)
