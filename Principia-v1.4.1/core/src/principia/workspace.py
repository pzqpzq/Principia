...
class Workspace:
    ...
    def __init__(
        ...
        *,
        ...
        self._working_directory_root = (
            ...
            else (self.root.parent if layout == "project" else self.root)
        )
        self._local_data_root = (
            Path(local_data_root).expanduser().resolve()
            if local_data_root is not None
            else self._working_directory_root
            / ("local_data" if layout == "project" else "Principia Local Data")
        )
        self._local_data_root.mkdir(parents=True, exist_ok=True, mode=0o700)
        if layout == "project":
            os.chmod(self.root, 0o700)
            os.chmod(self._local_data_root, 0o700)
            self._write_working_directory_manifest()
    ...
    def project(cls, root: str | Path = ".", **kwargs: Any) -> Workspace:
        ...
        return cls(
            ...
            layout="project",
            working_directory_root=project_root,
            local_data_root=project_root / "local_data",
            **kwargs,
        )

    @classmethod
    def working_directory(cls, root: str | Path, **kwargs: Any) -> Workspace:
        """Open the canonical v1.4 working-directory layout.

        ``workspace/`` owns application state and derived Principles.
        ``local_data/`` owns raw private source material and may be removed
        without deleting already extracted Principles.
        """

        return cls.project(root, **kwargs)

    def _write_working_directory_manifest(self) -> None:
        manifest = self._working_directory_root / ".principia-working-directory.json"
        body = (
            json.dumps(
                {
                    "schema_version": "principia-working-directory-v1",
                    "workspace": "workspace",
                    "local_data": "local_data",
                    "principles": "workspace/principles",
                    "raw_data_boundary": (
                        "Removing local_data does not remove derived Principles from workspace."
                    ),
                },
                ensure_ascii=False,
                indent=2,
        ...
        if manifest.exists() and manifest.read_bytes() == body:
            return
        descriptor, temporary = tempfile.mkstemp(
            prefix=".principia-layout.", dir=self._working_directory_root
        )
        try:
    ...
    @property
    def working_directory_root(self) -> Path:
    ...
    @property
    def principles_dir(self) -> Path:
        return self.root / "principles"
    ...
    def _ensure_visible_workspace(self) -> None:
        ...
        if self.layout == "project":
            # These are product boundaries, not implementation details.  Create
            # the durable Principle export directory on first open so a user
            # can understand the layout before running any extraction.
            self.principles_dir.mkdir(parents=True, exist_ok=True, mode=0o700)
            os.chmod(self.principles_dir, 0o700)
