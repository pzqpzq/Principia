...
class LocalCorpusIngestor:
    ...
    def ingest(
        ...
        *,
        ...
        progress_callback: Callable[[int, int, LocalSourceReport | None], bool | None]
        | None = None,
    ) -> WorkList:
        ...
        if progress_callback is not None:
            progress_callback(0, len(candidates), None)
        ...
        for index, path in enumerate(candidates, start=1):
            ...
            if work is not None:
                works.append(work)
            if progress_callback is not None:
                should_continue = progress_callback(index, len(candidates), report)
