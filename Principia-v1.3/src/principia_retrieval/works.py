...
def merge_work(left: dict[str, Any], right: dict[str, Any]) -> dict[str, Any]:
    ...
    metadata["has_peer_reviewed"] = any(
        [
            bool(left_metadata.get("is_peer_reviewed") or left_metadata.get("has_peer_reviewed")),
            bool(right_metadata.get("is_peer_reviewed") or right_metadata.get("has_peer_reviewed")),
        ]
    )
...
def source_quality(work: dict[str, Any]) -> tuple[int, int, int]:
    ...
    return (
        peer,
        {"crossref": 5, "europe_pmc": 4, "openalex": 3, "semantic_scholar": 2, "arxiv": 1}.get(
            source, 0
        ),
