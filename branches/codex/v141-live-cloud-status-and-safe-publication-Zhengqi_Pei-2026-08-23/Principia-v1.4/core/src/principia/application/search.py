...
from ..cloud import CloudRegistry
...
class PrincipleSearchService:
    ...
    def __init__(self, cloud: CloudRegistry, workspace: V14WorkspaceRepository) -> None:
        self.cloud = cloud
        ...
        *,
        ...
        if scope == "combined":
            ...
            with ThreadPoolExecutor(max_workers=2, thread_name_prefix="principia-search") as pool:
                global_future = pool.submit(self.cloud.search, query, limit=resolved_limit)
            ...
            global_rows = self.cloud.search(query, limit=resolved_limit)
        else:
