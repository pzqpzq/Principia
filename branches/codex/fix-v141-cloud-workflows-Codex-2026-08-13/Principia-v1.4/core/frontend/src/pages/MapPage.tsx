...
export function MapPage() {
  ...
  const principlePage = useQuery({
    queryKey: ["principle-cards", scope, q, area, packageId, goalId, sourceId, claimType, evidenceStatus, humanReview, minimumSupport, relationFilter, contradictions, sort, pageNumber],
  ...
  const collectionTitle = activeGoal?.title
    ?? activeFolder?.title
  ...
  return <div className="page explorer-page">
    <PageHeader
      eyebrow={scope === "global" ? "Downloaded scientific knowledge" : scope === "combined" ? "Private and downloaded scientific knowledge" : "Private scientific knowledge"}
      ...
      <main className="explorer-results">
        ...
        {!graphMode ? <section className="principle-card-grid" aria-label="Principle cards">{cards.map((card) => <article className={`principle-card ${selectedId === card.id ? "selected" : ""}`} key={card.id}>
          <button className="principle-card-open" onClick={() => updateParams({ selected: card.id })} aria-label={`Inspect ${card.title}`}>
            <div className="principle-card-top"><span className={`source-badge ${card.source}`}>{card.source === "local" ? "Local" : "Global"}</span><span>{claimLabels[card.claim_type] ?? card.claim_type.replaceAll("_", " ")}</span></div>
