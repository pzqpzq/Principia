            ...
            def retrieval_callback(stage: str, payload: dict[str, Any]) -> None:
                if stage == "query_plan":
                    ...
                    run.update(
                        ...
                        provisional_results=payload.get("provisional_results") or [],
                    )
...
def is_peer_reviewed_work(work: WorkItem) -> bool:
    ...
    if isinstance(value, bool):
        return value and bool(peer_reviewed_venue(work))
...
def peer_reviewed_venue(work: WorkItem) -> str:
    recorded_venue = clean_text(str(work.metadata.get("peer_reviewed_venue") or ""))
    venue = recorded_venue or clean_text(work.venue)
    if not venue:
        return ""
    venue_key = normalize_key(venue)
    if venue_key in NON_PUBLICATION_VENUES or any(
        marker in venue_key for marker in NON_PUBLICATION_VENUE_MARKERS
    ):
        return ""
    publication_type = normalize_key(
        str(work.metadata.get("publication_type") or work.metadata.get("type") or "")
    )
    if recorded_venue or work.metadata.get("is_peer_reviewed"):
        return venue
    if publication_type in PEER_REVIEWED_TYPES:
        return venue
    # Semantic Scholar can merge an arXiv copy with its conference or journal
    # record while retaining ``type=preprint``. Prefer the real venue over the
    # repository that happened to supply the downloadable copy.
    if normalize_key(work.source) == "semantic scholar" and _looks_like_publication_venue(venue):
        return venue
    return ""


def _looks_like_publication_venue(venue: str) -> bool:
    venue_key = normalize_key(venue)
    if not venue_key or venue_key in NON_PUBLICATION_VENUES:
        return False
    return not any(marker in venue_key for marker in NON_PUBLICATION_VENUE_MARKERS)
