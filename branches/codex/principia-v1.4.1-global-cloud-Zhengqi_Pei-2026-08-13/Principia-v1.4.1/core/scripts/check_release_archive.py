...
_WHEEL_NAME_RE = re.compile(
    r"^principia_ai-(?P<version>[0-9]+(?:\.[0-9]+){2}(?:[A-Za-z0-9.-]*)?)-py3-none-any\.whl$"
)
_SDIST_NAME_RE = re.compile(
    r"^principia_ai-(?P<version>[0-9]+(?:\.[0-9]+){2}(?:[A-Za-z0-9.-]*)?)\.tar\.gz$"
)
...
def _logical_path(member_name: str) -> str:
    ...
    if parts and re.fullmatch(r"principia_ai-[0-9]+(?:\.[0-9]+){2}(?:[A-Za-z0-9.-]*)?", parts[0]):
        parts = parts[1:]
    return "/".join(parts)
...
def _validate_archive_pair(archives: tuple[Path, ...]) -> None:
    ...
    if len(wheel_matches) != 1:
        if not wheel_matches and len(sdist_matches) == 1:
            ...
            details.append(f"missing principia_ai-{version}-py3-none-any.whl")
        else:
    ...
    if len(sdist_matches) != 1:
        if not sdist_matches and len(wheel_matches) == 1:
            ...
            details.append(f"missing principia_ai-{version}.tar.gz")
        else:
