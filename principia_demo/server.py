...
class PrincipiaRequestHandler(BaseHTTPRequestHandler):
    server_version = "PrincipiaDemo/0.1"
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if parsed.path.startswith("/api/v2/"):
                ...
                result = self.engine.delete_project(str(payload.get("field_id") or ""))
    ...
    def _handle_v2_post(self, path: str, payload: dict) -> dict:
        ...
        if path == "/api/v2/research/start":
            ...
            target_works = int(payload.get("target_works") or 50)
    ...
    def _handle_v1_post(self, path: str, payload: dict) -> dict:
        ...
        if path == "/api/v1/project/delete":
            return self.engine.delete_project(str(payload.get("field_id") or ""))
        ...
        if path == "/api/v1/import/v0":
            ...
            return self.engine.import_v0_store(str(payload.get("source_db_path") or default_source))
    ...
    def _handle_v2_get(self, path: str, params: dict[str, list[str]]) -> None:
        ...
        if path in {"/api/v2/research/status", "/api/v2/llm/status", "/api/v2/my-idea/generate/status", "/api/v2/my-idea/regenerate/status"}:
            ...
            if not run:
                ...
                return
            self._send_json({"run": run, "summary": self.engine.v2_project_summary(str(run.get("field_id") or field_id))})
            return
        if path == "/api/v2/project/summary":
            self._send_json(self.engine.v2_project_summary(field_id=field_id, query=query))
            return
    ...
    def _handle_v1_get(self, path: str, params: dict[str, list[str]]) -> None:
        ...
        if path == "/api/v1/project/summary":
            self._send_json(self.engine.project_summary(field_id, query=query))
            return
        if path == "/api/v1/project/tab":
            self._send_json(
                self.engine.build_project_tab(
                    field_id,
...
def run_server(host: str = "127.0.0.1", port: int = 8790) -> None:
    ...
    print(f"Principia demo running at http://{host}:{port}")
    try:
        httpd.serve_forever()
    except KeyboardInterrupt:
        print("\nStopping Principia demo.")
