...
subto DistCalc : forall <d> in D without {maxDiameter-1}: forall <s,t> in F : dist[s,t,d+1] <= dist[s,t,d] + sum <k> in N without {s,t} : dist[min(s,k),max(s,k),d] * dist[min(k,t),max(k,t),0];
